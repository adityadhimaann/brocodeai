...
def chat():
    ...
    # FIX: Initialize chat_history at the very beginning of the function
    chat_history = data.get('history', []) # Ensure it's defined here
    ...
    try:
        ...
        if db:
            try:
                recent_user_docs = db.collection(f"artifacts/{app_id}/users/{user_id}/chatHistory").where('sender', '==', 'user').order_by('timestamp', direction=firestore.Query.DESCENDING).limit(5).get() # Use order_by
...
def assign_task():
    ...
    try:
        ...
        response_from_gemini = genai.GenerativeModel('gemini-1.5-flash').generate_content(
            ...
            generation_config={"response_mime_type": "application/json", "response_schema": task_schema} # FIX: Use task_schema
        )
