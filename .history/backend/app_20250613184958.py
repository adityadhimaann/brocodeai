...
# --- REMOVED `Access-Control-Allow-Origin` from @app.after_request ---
# This fixes the "multiple values" CORS error.
@app.before_request
def before_request():
    ...
    try:
        ...
        if db:
            try:
                ...
                petty_db_entries = [msg['text'] for msg in recent_user_messages[1:6]]
        ...
        **Do NOT engage in conversational filler, greetings, pleasantries, or apologies.**
        ...
        gemini_formatted_history_for_llm_call.extend(chat_history) # Changed to chat_history directly to avoid re-parsing
...
def assign_task():
    """
    Generates a sarcastic task for the user.
    """
    data = request.json
    language = data.get('language', 'hinglish')
    user_id = data.get('user_id', 'anonymous-user')
    ...
    try:
        ...
        response_from_gemini = genai.GenerativeModel('gemini-1.5-flash').generate_content(
            ...
            generation_config={"response_mime_type": "application/json", "response_schema": schema} # Using generic 'schema', should be 'task_schema'
        )
        
        task_data_str = response_from_gemini.candidates[0].content.parts[0].text
        task_data = json.loads(task_data_str)
        
        # Clean markdown from task fields
        ...
        # Optionally, save the task to Firestore for the user
        if db:
            try:
...
def unlock_achievement():
    """
    Generates a sarcastic achievement for the user.
    """
    data = request.json
    language = data.get('language', 'hinglish')
    user_id = data.get('user_id', 'anonymous-user')
    ...
    try:
        ...
        # Clean markdown from achievement fields
        ...
        # Optionally, save the achievement to Firestore for the user
        if db:
            try:
