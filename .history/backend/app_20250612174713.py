        ...
        if tts_client:
            try:
                ...
                audio_config = tts.AudioConfig(
                    audio_encoding=tts.AudioEncoding.MP3 # Ensure this matches the actual content
                )
                ...
                # --- NEW DEBUG PRINT TO SAVE FILE ---
                # Check if content is bytes and non-empty before saving
                if isinstance(tts_response.audio_content, bytes) and tts_response.audio_content:
                    with open("chat_audio_output.mp3", "wb") as f:
                        f.write(tts_response.audio_content)
                    print(f"DEBUG: Saved chat_audio_output.mp3 to backend folder. Size: {len(tts_response.audio_content)} bytes.")
                else:
                    print("WARNING: tts_response.audio_content is not valid bytes or is empty for chat.")
                # --- END NEW DEBUG PRINT ---
...
def speak_text():
    ...
    try:
        ...
        tts_response = tts_client.synthesize_speech(input=s_input, voice=voice, audio_config=audio_config)
        audio_content_bytes = tts_response.audio_content # Get raw bytes
        audio_data_base64 = base64.b64encode(audio_content_bytes).decode('utf-8')
        
        # --- NEW DEBUG PRINT TO SAVE FILE FOR VERIFICATION ---
        if isinstance(audio_content_bytes, bytes) and audio_content_bytes:
            with open("temp_speak_text_output.mp3", "wb") as f:
                f.write(audio_content_bytes)
            print(f"DEBUG: Saved temp_speak_text_output.mp3 to backend folder. Size: {len(audio_content_bytes)} bytes.")
        else:
            print("WARNING: tts_response.audio_content is not valid bytes or is empty for /speak_text.")
        # --- END NEW DEBUG PRINT ---
