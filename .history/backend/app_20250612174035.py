...
app = Flask(__name__)
# Initialize CORS for all origins and methods here.
# This single line handles most CORS headers automatically for all routes.
CORS(app)


# --- CORS Preflight and Header Handling ---
# This ensures all responses (including OPTIONS preflights) have the correct CORS headers
@app.before_request
def before_request():
    if request.method == 'OPTIONS':
        resp = make_response()
        # Access-Control-Allow-Origin is handled by CORS(app) itself.
        # Adding it manually here would duplicate it.
...
def after_request(response):
    # Do NOT add 'Access-Control-Allow-Origin' here, as CORS(app) already does it.
    # Adding it would result in the "multiple values" error.
...
# --- TTS Voice Mappings ---
# Researching Google Cloud TTS voices is key here:
# https://cloud.google.com/text-to-speech/docs/voices
# I'm choosing some common WaveNet voices that tend to sound natural.
# For 'sarcastic', we'll rely on the LLM's text output combined with a neutral/slightly deep voice.
# For 'hot', a distinct male/female voice.
TTS_VOICES_BY_STYLE = {
    'hinglish': {
        'default': {'name': 'hi-IN-Wavenet-B', 'gender': tts.SsmlVoiceGender.MALE}, # Hindi Male, good for Hinglish
        'sarcastic': {'name': 'hi-IN-Wavenet-B', 'gender': tts.SsmlVoiceGender.MALE}, # Same, sarcasm is in text
        'hot_male': {'name': 'hi-IN-Wavenet-B', 'gender': tts.SsmlVoiceGender.MALE}, # Male voice
        'hot_female': {'name': 'hi-IN-Wavenet-A', 'gender': tts.SsmlVoiceGender.FEMALE}, # Female voice
    },
    'en': {
        'default': {'name': 'en-IN-Wavenet-B', 'gender': tts.SsmlVoiceGender.MALE},
        'sarcastic': {'name': 'en-US-Wavenet-J', 'gender': tts.SsmlVoiceGender.MALE}, # US Male, often good for dry tone
        'hot_male': {'name': 'en-US-Wavenet-E', 'gender': tts.SsmlVoiceGender.MALE}, # Different US Male
        'hot_female': {'name': 'en-US-Wavenet-C', 'gender': tts.SsmlVoiceGender.FEMALE}, # US Female
    },
    'hi': { # Hindi (Devanagari)
    ...
    # Add other languages or voice styles as needed
}
...
def chat():
    ...
    if not gemini_model: # TTS client can be optional if voice features aren't critical path
        return jsonify({'error': 'Gemini model not initialized. Cannot process chat.'}), 500

    try:
        # Determine language instruction for Gemini prompt
        ...
        # Enhanced persona prompt for deeper, more "abusive" sarcasm
        ...
        if tts_client:
            try:
                # Determine TTS language code (Hindi for Hinglish)
                tts_lang_code = 'hi' if selected_language == 'hinglish' else selected_language
                
                # Look up voice settings based on selected_language and voice_style
                ...
                print(f"DEBUG: Calling Google Cloud Text-to-Speech for chat response: Language={tts_lang_code}, Voice={voice_name}, Gender={voice_gender}, Text='{bot_response_text[:50]}...'")
                ...
                print(f"DEBUG: Successfully synthesized speech for chat. Base64 length: {len(audio_data_base64)} bytes.")

            except GoogleAPIError as e:
                print(f"WARNING: Google Text-to-Speech API error during chat synthesis: {e.message}")
                print("Ensure Text-to-Speech API is enabled and GOOGLE_APPLICATION_CREDENTIALS is valid.")
            except Exception as e:
                print(f"WARNING: Unexpected error during Text-to-Speech synthesis for chat: {e}")
        else:
            print("WARNING: Text-to-Speech client not initialized. Skipping audio synthesis for chat.")
    ...
    try:
        ...
        schema = {
            ...
            "items": {
                "type": "OBJECT",
                "properties": {
                    "type": "STRING", # Corrected type for 'type'
                    "content": "STRING"
                },
...
def generate_brocode_meme():
    ...
    try:
        ...
        # --- Image Generation (Conceptual Placeholder) ---
        # For actual image generation with Imagen, you'd use Vertex AI SDK.
        # For this demo, we'll use placehold.co to dynamically generate an image based on description.
        placeholder_width = 500
        placeholder_height = 400
        # Use part of the caption/description for the placeholder text for relevance
        ...
        image_url = f"https://placehold.co/{placeholder_width}x{placeholder_height}/1A202C/A0AEC0?text={placeholder_text}" # Dark background, light text
...
def roast_me():
    """
    Generates a brutal, sarcastic, personalized (generalized) roast.
    """
    data = request.json
    language = data.get('language', 'hinglish')
...
def speak_text():
    """
    Converts given text to speech using Google Cloud TTS, with selected language and voice style.
    """
    data = request.json
    ...
    if not tts_client:
        print("ERROR: Text-to-Speech client not initialized. Cannot synthesize speech.")
    ...
    try:
        ...
        print(f"DEBUG: Calling Google Cloud Text-to-Speech for /speak_text: Language={tts_lang_code}, Voice={voice_name}, Gender={voice_gender}, Text='{text_to_speak[:50]}...'")
        tts_response = tts_client.synthesize_speech(input=s_input, voice=voice, audio_config=audio_config)
        audio_data_base64 = base64.b64encode(tts_response.audio_content).decode('utf-8')
        print(f"DEBUG: Successfully synthesized speech for /speak_text. Base64 length: {len(audio_data_base64)} bytes.")

        return jsonify({'audio': audio_data_base64})

    except GoogleAPIError as e:
        print(f"ERROR: Google TTS API Error (speak_text): {e.message}")
        print("Please check Google Cloud Text-to-Speech API status, quotas, and GOOGLE_APPLICATION_CREDENTIALS.")
        return jsonify({'error': f'Google TTS API error: {e.message}'}), 500
    except Exception as e:
        print(f"ERROR: Backend error (speak_text): {e}")
        return jsonify({'error': f'An unexpected error occurred during speak_text: {str(e)}'}), 500
