...
# --- CRITICAL: VERIFY THIS ENDPOINT WITH SARVAM AI'S LATEST DOCS ---
# This was the source of the 404. Find the EXACT URL for TTS Synthesis.
SARVAM_AI_TTS_ENDPOINT = os.getenv("SARVAM_AI_TTS_ENDPOINT", "https://api.sarvam.ai/text-to-speech") # <-- UPDATE THIS IF DIFFERENT
...
# --- CRITICAL: UPDATE THESE VOICE MAPPINGS WITH ACTUAL SARVAM AI VOICE IDs/MODELS ---
# These are PLACEHOLDERS. You MUST find the correct identifiers from Sarvam AI docs.
# For Bhashini compliant APIs, 'serviceId' is often the model ID, and 'voice_id' or 'speaker_id' selects within it.
# Check their API playground or docs for valid 'target_language_code', 'serviceId', 'voice_id', 'gender' values.
SARVAM_AI_VOICES_BY_STYLE = {
    ...
    # Add other languages supported by Sarvam AI and your app.
}
...
def synthesize_sarvam_ai_speech(text, language, voice_style):
    ...
    # --- IMPORTANT: Parameters depend on what Sarvam AI's *actual* API requires ---
    ...
    if not sarvam_target_language_code or not sarvam_service_id:
        print(f"ERROR: Missing essential Sarvam AI voice parameters for language '{language}' and style '{voice_style}'. Check SARVAM_AI_VOICES_BY_STYLE map.")
        return None

    # This payload structure is a common ULCA/Bhashini pattern.
    # You might need to adjust it based on your specific Sarvam AI API version/documentation.
    payload = {
        "input": [
            {
                "source": text,
        ...
        "config": {
            "language": {
                "sourceLanguage": sarvam_target_language_code, # Use target_language_code for source here
            },
            "gender": sarvam_gender,
            "voice_id": sarvam_voice_id, # Can be optional if not used by the model
            "serviceId": sarvam_service_id, # This is usually the model ID to select
            "audioFormat": "mp3"
        },
        "control": {
            "speech_speed": 1.0 # Example control parameter, adjust if needed
        }
    }
    ...
    try:
        ...
        response = requests.post(SARVAM_AI_TTS_ENDPOINT, headers=headers, json=payload, timeout=15) # Increased timeout
        ...
        # --- UPDATED: Robustly extract base64 audio content from known Sarvam/Bhashini patterns ---
        if response_json and isinstance(response_json, dict):
            if response_json.get('audioContent'): # Common for Bhashini/ULCA
                audio_content_base64 = response_json['audioContent']
            elif response_json.get('audio'): # Direct 'audio' key
        ...
        if audio_content_base64:
            ...
            print(f"WARNING: Sarvam AI TTS response did not contain audio content in expected format for text: '{text[:50]}...'")
            return None
...
def speak_text():
    ...
    try:
        voice_settings = SARVAM_AI_VOICES_BY_STYLE.get(language, {}).get(voice_style, DEFAULT_SARVAM_VOICE_SETTINGS)
        sarvam_service_id = voice_settings.get('serviceId') # This is the main model ID, often
        sarvam_voice_id = voice_settings.get('voice_id') # Specific voice within serviceId

        sarvam_source_language = SARVAM_LANG_MAP.get(language, 'en')

        if not sarvam_service_id: # Check for the serviceId (model identifier)
            print(f"ERROR: No Sarvam AI serviceId found for language '{language}' and voice style '{voice_style}'. Check SARVAM_AI_VOICES_BY_STYLE map.")
            return jsonify({'error': 'Sarvam AI voice model not found for selected style.'}), 500

        # --- UPDATED PAYLOAD STRUCTURE based on screenshot and common Bhashini API patterns ---
        # The screenshot implies a simpler payload for the /text-to-speech endpoint.
        # It's possible "speaker" and "pitch" are optional query params or body parameters.
        # Let's try the simplest form first based on the screenshot, then add optional parameters if needed.
        payload = {
            "text": text_to_speak,
            "target_language_code": sarvam_source_language, # Use this as per screenshot.
            # "speaker": sarvam_voice_id, # This might be an optional parameter, check docs
            # "pitch": 1.0, # Optional, check docs
            # "pace": 1.0, # Optional, check docs
            # "loudness": 1.0, # Optional, check docs
            # "speech_s": 1.0, # Optional, check docs
            # "audioFormat": "mp3" # Output format should be requested, but endpoint implies it.
        }
        # If the API requires 'config' and 'input' wrappers as before, revert to that structure.
