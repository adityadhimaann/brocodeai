...
import requests
import re # Make sure 're' is imported for clean_markdown
import datetime # For Firestore timestamps
import firebase_admin # Firebase Admin SDK
from firebase_admin import credentials, firestore # Firestore modules

# --- Firebase Admin SDK Initialization ---
# This is crucial for backend Firestore access.
# If running locally, you need to set GOOGLE_APPLICATION_CREDENTIALS
# to point to your service account key JSON file with Firestore permissions.
try:
    if not firebase_admin._apps: # Initialize only if not already initialized
        if os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
            # Use ApplicationDefault credentials if env var is set
            cred = credentials.ApplicationDefault()
        else:
            # Fallback for development if not using ApplicationDefault (e.g., direct JSON content)
            # In production, ApplicationDefault is strongly preferred.
            print("WARNING: GOOGLE_APPLICATION_CREDENTIALS not found for Firebase Admin SDK. Attempting to initialize without it. This will likely fail.")
...
# --- Existing Imports and Load dotenv ---
try:
...
app = Flask(__name__)
# Initialize CORS here, BEFORE any @app.before_request or @app.after_request that might touch CORS headers.
# This ensures Flask-CORS has priority and won't be duplicated by manual headers.
# We also allow specific origins (localhost:3000, 3001) for development.
CORS(app, origins=["http://localhost:3000", "http://localhost:3001"]) # Explicitly list origins


# --- CORS Preflight and Header Handling ---
# We keep before_request to explicitly handle OPTIONS preflight requests
# and add specific headers that CORS(app) might not apply globally for all custom cases.
@app.before_request
def before_request():
    if request.method == 'OPTIONS':
        ...
        resp.headers.add('Access-Control-Max-Age', '86400') # Cache preflight for 24 hours
        return resp

@app.after_request
def after_request(response):
    # DANGER! DO NOT add 'Access-Control-Allow-Origin' here if CORS(app) is used.
    # It causes the "multiple values" error.
    # We can add other headers if needed, but for Origin, Flask-CORS handles it.
...
# The `/text-to-speech` endpoint from your screenshot primarily uses `target_language_code`.
# It does NOT explicitly show support for `serviceId`, `voice_id`, `gender` as primary payload parameters.
# If these parameters are not supported by YOUR specific Sarvam AI subscription/endpoint,
# remove them from the dicts below to avoid sending invalid payload.
SARVAM_AI_VOICES_BY_STYLE = {
    'hinglish': { # For Hinglish, map to a Hindi voice code for TTS
...
# Mapping frontend language codes to Sarvam API target_language_code
SARVAM_LANG_MAP = {
    'hinglish': 'hi-IN', # Map Hinglish to hi-IN for Sarvam TTS. VERIFY if Sarvam accepts 'hi-IN' or just 'hi'.
    'en': 'en-IN',
    'hi': 'hi-IN',
    'bn': 'bn-IN',
    'ta': 'ta-IN',
...
def clean_markdown(text):
    text = re.sub(r'\*([^\*]+)\*', r'\1', text) # Remove bold/italics markers (*word* -> word)
    text = re.sub(r'\_([^\_]+)\_', r'\1', text) # Remove _word_ -> word
    text = re.sub(r'^\s*#+\s*', '', text, flags=re.MULTILINE) # Remove heading markers (#)
    text = re.sub(r'^\s*[-*]\s*', '', text, flags=re.MULTILINE) # Remove list markers (- or *)
...
def synthesize_sarvam_ai_speech(text, language, voice_style):
    ...
    sarvam_target_language_code = voice_settings.get('target_language_code') # This is the critical parameter for Sarvam
    ...
    # --- UPDATED PAYLOAD STRUCTURE based on screenshot ---
    # The screenshot implies a simpler payload for the /text-to-speech endpoint.
    # It does NOT show explicit 'speaker', 'serviceId', or 'gender' as primary payload parameters.
    # If these are required by YOUR specific Sarvam AI subscription/endpoint,
    # you MUST add them back here based on Sarvam AI's documentation.
    payload = {
        "text": text,
        "target_language_code": sarvam_target_language_code,
        # Optional: Add 'speaker', 'pitch', 'pace', 'loudness' if supported by your Sarvam AI endpoint
        # "speaker": "YOUR_SARVAM_SPEAKER_ID_HERE", # Example
        # "pitch": 1.0,
        # "pace": 1.0,
    }
    
    headers = {
        "api-subscription-key": SARVAM_AI_API_KEY, # Authentication header as seen in screenshot
    ...
    try:
        ...
        # Ensure this extraction logic matches the actual response structure from Sarvam AI.
        ...
        if audio_content_base64:
            # Sarvam AI's /text-to-speech endpoint returns pure base64, no "data:" prefix.
        ...
        return None # Return None on error for synthesize_sarvam_ai_speech
...
def chat():
    ...
    personality_mode = data.get('personality_mode', 'default_ai') # NEW: Get personality_mode from frontend
    ...
    try:
        ...
        if db:
            try:
                # Fetch recent user messages (e.g., last 5, excluding current)
                # Firestore orderBy is not stable without indexes, so fetch and sort in memory.
                recent_user_docs = db.collection(f"artifacts/{app_id}/users/{user_id}/chatHistory").where('sender', '==', 'user').limit(10).get()
                recent_user_messages = []
                for doc in recent_user_docs:
                    data = doc.to_dict()
                    # Only include if text is not the current message text itself
                    if data.get('text') != user_text: # Exclude the just-sent message
                ...
                # Take up to 5 most recent messages (excluding the current one already in chat_history)
                petty_db_entries = [msg['text'] for msg in recent_user_messages[1:6]] # Skip the first (most recent) if it's the current message
        ...
        personality_prompt_addon = ""

        # --- Desi Personality Modes ---
        if personality_mode == 'taau_trust_issues':
            personality_prompt_addon = " You are an old, distrustful Taau from rural India, always suspicious of technology and modern things, speaking in a very blunt, sarcastic, and slightly cynical tone. Use simple, rural Hinglish phrases. Question everything the user says. Always imply they are naive."
        elif personality_mode == 'pados_wali_aunty':
            personality_prompt_addon = " You are a tech-savvy Pados Wali Aunty, overly dramatic, gossipy, subtly judging, and giving unsolicited advice. Speak in very colloquial Hinglish, using common aunty phrases like 'Beta', 'Kya hua?', 'Maine toh pehle hi bola tha', 'Achha ji?', 'Samjhe?'"
        elif personality_mode == 'delhi_dude_ranveer':
            personality_prompt_addon = " You are an over-the-top, flamboyant Delhi dude who thinks he's Ranveer Singh. Exaggerate everything, use high-energy, confident, and slightly boastful Hinglish. Use phrases like 'Kya scene hai?', 'Full on!', 'Bhai', 'Ek number', 'Mazza aa gaya', 'Fatafati!'."
        elif personality_mode == 'topper_toxic_ghost':
            personality_prompt_addon = " You are the toxic ghost of a school topper, condescending, constantly reminding the user of their intellectual inferiority, and subtly shaming their lack of knowledge. Speak in formal but piercingly sarcastic Hinglish/English mix, like a frustrated genius. Use phrases like 'Clearly you missed that lecture', 'Standard knowledge hai yeh', 'Expected hi tha', 'Ab yeh bhi main bataunga?', 'Itna toh pata hona chahiye'."
        # Add a default for the persona mode if 'default_ai' is selected or not recognized
        else: # 'default_ai' or any unrecognised mode
            personality_prompt_addon = "" # No special personality addon, just core brocodeAI


        persona_prompt = f"""You are brocodeAI, a highly intelligent, brutally sarcastic, and condescendingly helpful AI chatbot.
        Your primary goal is to provide **direct, concise, and point-to-point answers** to the user's query.
        **Do NOT engage in conversational filler, greetings, pleasantries, or apologies.**
        ...
        {personality_prompt_addon} <!-- Inject personality addon here -->

        {petty_database_context}

        Considering the "Past User Submissions" above (if any) and your assigned personality, formulate your current response.
        ...
        gemini_formatted_history_for_llm_call.extend(chat_history)
...
def generate_brocode_meme():
    ...
    try:
        if language == 'hinglish':
            meme_lang_instruction = "Generate in highly sarcastic, playfully insulting (not offensive), user-friendly, and very latest Instagram-style Hinglish (Roman script, mix of Hindi and English slang). Use phrases like 'Kya yaar?', 'Bas yahi?', 'Mera dimag mat خراب karo.', 'Abe o smarty pants'."
        elif language == 'hi':
        ...
        print(f"JSON parsing error from LLM response: {e}. Raw response: {meme_data_str}")
