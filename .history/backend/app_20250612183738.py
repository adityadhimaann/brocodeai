...
def synthesize_sarvam_ai_speech(text, language, voice_style):
    ...
    # --- FIX: Retrieve sarvam_target_language_code at the start ---
    ...
    sarvam_gender = voice_settings.get('gender') # Make sure this is retrieved too
    ...
    # If `speaker`, `gender`, `serviceId`, or `voice_id` are supported, they must be added back as optional parameters.
    payload = {
        "text": text,
        "target_language_code": sarvam_target_language_code,
        # Example of adding optional parameters ONLY IF your specific Sarvam AI endpoint supports them:
        # "speaker": sarvam_voice_id, # This might be an optional parameter, check docs
        # "gender": sarvam_gender, # This might be an optional parameter, check docs
        # "serviceId": sarvam_service_id, # This might be an optional parameter, check docs
...
def speak_text():
    ...
    try:
        voice_settings = SARVAM_AI_VOICES_BY_STYLE.get(language, {}).get(voice_style, DEFAULT_SARVAM_VOICE_SETTINGS)
        # Ensure sarvam_target_language_code is retrieved
        sarvam_target_language_code = voice_settings.get('target_language_code')

        # Check for mandatory parameters as defined by Sarvam AI's API
        if not sarvam_target_language_code:
        ...
        payload = {
            "text": text_to_speak, # This should be the full text
            "target_language_code": sarvam_target_language_code,
            # Add other optional parameters ONLY IF your specific Sarvam AI endpoint supports them
            # based on your current API subscription/model.
            # E.g., "speaker": voice_settings.get('voice_id'),
            # "serviceId": voice_settings.get('serviceId'),
            # "gender": voice_settings.get('gender')
        }
