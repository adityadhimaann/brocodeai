...
def chat():
    ...
    personality_mode = data.get('personality_mode', 'default_ai')
    ...
    try:
        ...
        if db:
            try:
                recent_user_docs = db.collection(f"artifacts/{app_id}/users/{user_id}/chatHistory").where('sender', '==', 'user').order_by('timestamp', direction=firestore.Query.DESCENDING).limit(5).get()
                recent_user_messages = []
                for doc in recent_user_docs:
                    data = doc.to_dict()
                    if data.get('text') != user_text:
                        recent_user_messages.append({'text': data['text'], 'timestamp': data['timestamp']})
                
                petty_db_entries = [msg['text'] for msg in recent_user_messages]
...
def generate_brocode_meme():
    ...
    try:
        ...
        return jsonify({'error': f'A service error occurred during brocode meme generation: {e.message}'}), 500
    except json.JSONDecodeError as e:
