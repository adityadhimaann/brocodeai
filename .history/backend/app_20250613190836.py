...
def chat():
    ...
    # NEW: Get persona mode from frontend
    selected_persona_mode = data.get('persona_mode', 'Default brocodeAI') # Default to standard brocodeAI
    ...
    try:
        ...
        if db:
            try:
                ...
                # Use limit(6) to fetch slightly more, then exclude the current message
                ...
                for doc in recent_user_docs:
                    data = doc.to_dict()
                    # Only include if timestamp exists and text is not the current message text itself
                    if data.get('timestamp') and data.get('text') != user_text:
                        recent_user_messages.append({'text': data['text'], 'timestamp': data['timestamp']})
                
                # Sort in memory by timestamp descending, then take up to 5
                recent_user_messages.sort(key=lambda x: x['timestamp'], reverse=True)
                petty_db_entries = [msg['text'] for msg in recent_user_messages[:5]] # Take up to 5 after sorting
        ...
        # --- UPDATED PROMPT with Persona Mode Logic ---
        ...
        **Do NOT engage in conversational filler, greetings, pleasantries, or apologies.**
        ...
        # Ensure chat_history is formatted as expected by Gemini: list of {'role': '...', 'parts': [{'text': '...'}]}
