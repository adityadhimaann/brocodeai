...
from firebase_admin import credentials, firestore # NEW: Firestore modules
...
try:
    if not firebase_admin._apps: # Initialize only if not already initialized
        ...
        if os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
            ...
            print("WARNING: GOOGLE_APPLICATION_CREDENTIALS not found. Attempting to initialize Firebase Admin without it.")
            # If your app needs a specific service account key JSON file directly:
            # service_account_info = json.loads(os.getenv("FIREBASE_SERVICE_ACCOUNT_KEY_JSON")) # If stored as JSON string
            # cred = credentials.Certificate(service_account_info)
            # For this context, we'll let ApplicationDefault handle it, expecting env var.
...
try:
    ...
    print(f"pip install google-cloud-speech google-generativeai flask flask-cors python-dotenv requests firebase-admin") # Added firebase-admin
...
SARVAM_AI_TTS_ENDPOINT = os.getenv("SARVAM_AI_TTS_ENDPOINT", "[https://api.sarvam.ai/text-to-speech](https://api.sarvam.ai/text-to-speech)")
...
def chat():
    ...
    # Assuming __app_id is globally available in Canvas environment,
    # and userId is passed from frontend.
    app_id = os.getenv('__app_id', 'default-app-id') # Fallback if not in env
    user_id = data.get('user_id', 'anonymous-user') # Get userId from frontend
    ...
    try:
        # --- Petty Database: Store User Input ---
        if db:
            try:
        ...
        # --- Petty Database: Retrieve Recent User Messages for Context ---
        ...
        if db:
            try:
                ...
                for doc in recent_user_docs:
                    data = doc.to_dict()
                    if data.get('timestamp'): # Only add if timestamp exists
                ...
                # Take up to 5 most recent messages (excluding the current one)
                petty_db_entries = [msg['text'] for msg in recent_user_messages[1:6]] # Exclude current msg, take 5
        ...
        # --- UPDATED PROMPT with Petty Database Context ---
        # Note: Chat history is formatted as a list of dictionaries with 'role' and 'parts' keys.
        # This is standard for Gemini's generate_content API.
        current_chat_history_for_llm = []
        for msg in chat_history:
            role = 'user' if msg.get('sender') == 'user' else 'model'
            current_chat_history_for_llm.append({'role': role, 'parts': [{'text': msg.get('text')}]})
        ...
        **Do NOT engage in conversational filler, greetings, pleasantries, or apologies unless explicitly asked.**
        ...
        {petty_database_context} <!-- Insert Petty Database Context here -->
        ...
        # Add the current chat history for short-term memory
        gemini_formatted_history_for_llm_call.extend(current_chat_history_for_llm) # Add existing chat history
        ...
        # --- Petty Database: Store AI Response ---
        if db:
            try:
...
def generate_brocode_meme():
    ...
    try:
        ...
        image_url = f"[https://placehold.co/](https://placehold.co/){placeholder_width}x{placeholder_height}/1A202C/A0AEC0?text={placeholder_text}"
