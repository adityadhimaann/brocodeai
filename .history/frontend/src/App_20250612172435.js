...
export default function ScrollingHumorPanel({ humorItems = [], direction = 'up', position = 'left', isLoading, error, onRefresh, onSpeakText, selectedLanguage, selectedVoiceStyle }) {
  ...
  const handleListenClick = async (content, itemId) => {
    ...
    if (currentlySpeakingId === itemId) {
      ...
      return; // Prevent multiple clicks on the same item while it's speaking
    }
    ...
    try {
      ...
      console.log(`[ScrollingHumorPanel] Successfully played audio for item: ${itemId}`);
    } catch (err) {
      console.error(`[ScrollingHumorPanel] Failed to play humor audio for item ${itemId}:`, err);
    } finally {
      // Use a small delay before clearing speakingId to ensure animation completes visually
      // or to prevent rapid re-enabling if audio is very short.
      setTimeout(() => {
            ...
            {displayItems.map((item, index) => (
              ...
              >
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  // Ensure disabled state correctly reflects if currently speaking
                  disabled={currentlySpeakingId === `${item.type}-${index}`}
                >
