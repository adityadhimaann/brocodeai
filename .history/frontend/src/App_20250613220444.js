...
const MotionButton = forwardRef((props, ref) => (
...
export default function App() {
  ...
  const [selectedPersonaMode, setSelectedPersonaMode] = useState('default');
  ...
  const fetchHumor = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:5002/get_humor', {
        ...
        body: JSON.stringify({ language: selectedLanguage, persona_mode: selectedPersonaMode }),
      });
      if (!response.ok) {
  ...
  }, [selectedLanguage, selectedPersonaMode]);
  ...
  }, [selectedLanguage, selectedPersonaMode, fetchHumor, fetchUnsolicitedAdvice]);
      ...
      unsubscribeAuth = onAuthStateChanged(authInstance, async (user) => {
        if (user) {
          ...
          setLoggedInUser({
            ...
            profilePic: `https://placehold.co/150x150/FF00FF/FFFFFF?text=${user.uid.substring(0, 2)}`
          });
        } else {
          ...
          if (__initial_auth_token) {
            try {
              ...
              try {
                ...
                setLoggedInUser({
                  username: anonUserCredential.user.uid.substring(0, 8) + ' (Anon)',
                  profilePic: `https://placehold.co/150x150/CCCCCC/000000?text=${anonUserCredential.user.uid.substring(0, 2)}`
                });
  ...
  const speakArbitraryText = useCallback(async (text, lang, voiceStyle) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5002/speak_text', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: text, language: lang, voice_style: voiceStyle, persona_mode: selectedPersonaMode }),
      });
      if (!response.ok) {
        const errorData = await response.json();
  ...
  }, [playAudioFromBase64, displayMessageBox, selectedPersonaMode]);
    ...
    if (app) {
      ...
      signInAnonymously(authInstance)
        .then(userCredential => {
          ...
          setUserId(user.uid);
          setLoggedInUser({
            username: user.uid.substring(0, 8) + ' (Anon)',
            profilePic: `https://placehold.co/150x150/CCCCCC/000000?text=${user.uid.substring(0, 2)}`
          });
  ...
  const assignSarcasticTask = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:5002/assign_task', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId, language: selectedLanguage, persona_mode: selectedPersonaMode }),
      });
      if (!response.ok) {
        const errorData = await response.json();
      ...
      }, [userId, selectedLanguage, selectedPersonaMode, displayMessageBox]);
      ...
      const mainContentMxClass = `md:ml-[var(--panel-width-val)] md:mr-[var(--panel-width-val)]`;
      ...
      return (
        <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
          ...
          <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'ml-64' : 'ml-0'} ${mainContentMxClass} z-20 p-2 sm:p-4`}>
            <ChatHeader
              languages={languages}
              selectedLanguage={selectedLanguage}
