...
import { Loader2, Menu } from 'lucide-react'; // Import Menu icon for the SheetTrigger
...
import { Sheet, SheetContent, SheetTrigger, SheetClose } from './components/ui/sheet'; // Import Sheet and SheetContent, SheetTrigger, SheetClose
import { Button } from './components/ui/button'; // Import Button for SheetTrigger button
...
export default function App() {
  ...
  const handleLogin = useCallback((username, password) => {
    ...
    if (!app) {
        ...
        if (getApps().length === 0) { // Check if NO Firebase apps are initialized yet
            app = initializeApp(firebaseConfig);
            setFirebaseApp(app); // Store the new app instance
        } else {
            app = getApp(); // Get the default app if already initialized
            setFirebaseApp(app); // Store the app instance
        }
    }
    ...
    if (app) {
        ...
        signInAnonymously(authInstance)
            .then(userCredential => {
                ...
                setUserId(user.uid); // Update userId in state
