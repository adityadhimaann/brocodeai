...
import AchievementToast from './components/AchievementToast.js'; // NEW: Achievement Toast

// Firebase imports for client-side (authentication for userId)
import { initializeApp } from 'firebase/app';
import { getAuth, signInAnonymously, signInWithCustomToken, onAuthStateChanged } from 'firebase/auth';
// No direct Firestore client import on frontend for this feature; backend handles it.
...
// Default to empty strings if not defined in Canvas runtime
...
export default function App() {
  ...
  const [userId, setUserId] = useState(null); // The user ID for Firestore
  const [loggedInUser, setLoggedInUser] = useState(null); // For conceptual login
  ...
  // Novelty features: Task and Achievement
  const [currentTask, setCurrentTask] = useState(null); // { title: string, description: string }
  const [unlockedAchievement, setUnlockedAchievement] = useState(null); // { title: string, description: string }
    ...
    try {
        ...
        const unsubscribe = onAuthStateChanged(authInstance, async (user) => {
            if (user) {
                ...
                setUserId(user.uid);
                // For conceptual login, set a dummy user profile
                if (!loggedInUser) {
        ...
        displayMessageBox("Firebase initialization failed. Chat history might not save.", "error");
        setUserId(generateUUID()); // Fallback to random UUID if Firebase fails
    }
  }, [loggedInUser, displayMessageBox]); // Add loggedInUser to dependency to avoid re-auth loop with conceptual login
  ...
  const sendMessage = async () => {
    if (input.trim() === '' || !userId) return; // Ensure userId exists before sending
    ...
    try {
      const response = await fetch('http://localhost:5002/chat', {
        ...
        body: JSON.stringify({
          ...
          app_id: __app_id, // Pass app_id to backend
        }),
      });
  ...
  const playAudioFromBase64 = useCallback(async (base64Audio, source = "chat") => {
    ...
    try {
        ...
        if (e.name === "NotAllowedError" || e.name === "AbortError") {
            ...
            displayMessageBox("Audio format not supported. Please check backend audio encoding.", "error");
  ...
  const handleLogin = useCallback((username, password) => {
    ...
    if (auth) { // Only proceed if Firebase Auth is initialized
        signInWithCustomToken(auth, __initial_auth_token || 'dummy_token') // Use token if available, else a dummy
            .then((userCredential) => {
            ...
            .catch(async (error) => {
                ...
                try {
                    ...
                    displayMessageBox("Authentication failed. Invalid credentials or Firebase issue. Access denied.", "error");
                    setUserId(generateUUID()); // Fallback to local UUID
                }
            });
    } else {
        console.error("Firebase Auth not initialized during handleLogin.");
        displayMessageBox("Authentication system not ready. Try again later, or refresh.", "error");
        setUserId(generateUUID()); // Fallback to local UUID
    }
  }, [displayMessageBox, auth]);
  ...
  // NEW: Function to assign a sarcastic task
  const assignSarcasticTask = useCallback(async () => {
    ...
    try {
        ...
        const data = await response.json();
        setCurrentTask(data); // Assuming {title, description}
  ...
  // NEW: Function to unlock a sarcastic achievement
  const unlockSarcasticAchievement = useCallback(async () => {
    ...
    try {
        ...
        const data = await response.json();
        setUnlockedAchievement(data); // Assuming {title, description}
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out
        ...
        <ChatHeader
          ...
          onAssignTask={assignSarcasticTask} // Pass new task function
          onUnlockAchievement={unlockSarcasticAchievement} // Pass new achievement function
        />
