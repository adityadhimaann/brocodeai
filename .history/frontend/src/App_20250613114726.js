...
// Sidebar will be rendered directly inside SheetContent
...
// Import Loader2 and Menu directly into App.js
import { Loader2, Menu } from 'lucide-react'; // FIX: Added Menu to import

// Firebase imports for client-side
import { initializeApp, getApps, getApp } from 'firebase/app';
import { getAuth, signInAnonymously, signInWithCustomToken, onAuthStateChanged } from 'firebase/auth';

// NEW: Import ALL Shadcn Sheet components for use directly in App.js
import { Sheet, SheetContent, SheetTrigger, SheetClose } from './components/ui/sheet'; // FIX: Sheet, SheetContent, SheetTrigger, SheetClose explicitly imported
import { Button } from './components/ui/button'; // Import Button for SheetTrigger button (and other uses)
import { DialogTitle, DialogDescription } from './components/ui/dialog'; // For SheetContent's accessibility
...
export default function App() {
  ...
  const handleLogin = useCallback((username, password) => {
    ...
    // Ensure Firebase app instance is already initialized
    let app = firebaseApp;
    if (!app) {
        // If no app instance is set, try to get the default app or initialize
        if (getApps().length === 0) {
  ...
  // Unused variables for layout, removed comments
  // const panelWidth = 'w-64 md:w-72 lg:w-80';
  // const sidebarWidth = 'w-64';
  const mainContentMlClass = isSidebarOpen ? 'ml-64' : 'ml-0'; // Used directly
  const mainContentMxClass = `md:ml-[var(--panel-width-val)] md:mr-[var(--panel-width-val)]`; // This is used in JSX


  // Render a loading spinner until userId is determined
  if (userId === null) {
    return (
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      {/* Sidebar Component (Managed by Sheet in App.js) */}
      {/* The Sheet component manages its own open state and context for SheetTrigger/SheetClose */}
      <Sheet open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
        {/* SheetTrigger is here, making it a direct child of Sheet */}
        <SheetTrigger asChild>
          {/* This button is part of App.js's render, directly within Sheet's context */}
          <motion.button
            // No onClick={toggleSidebar} needed here, SheetTrigger handles it
            className="p-1 md:p-2 rounded-full bg-zinc-700 hover:bg-zinc-600 text-white focus:outline-none focus:ring-2 focus:ring-zinc-600 transition-colors duration-200 absolute top-3 left-3 md:hidden z-50"
        ...
        >
          ...
          <Sidebar
            // Sidebar no longer handles its own open/close state directly
            // Instead, it receives a simplified toggle function that sets isSidebarOpen in App.js
      ...
      {/* Main Content Area */}
      {/* Adjust margins based on isSidebarOpen for mobile overlay and fixed panels for desktop */}
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out
                       ${isSidebarOpen ? 'ml-64' : 'ml-0'} /* Mobile: pushes main content if open */
                       md:ml-[var(--panel-width-val)] md:mr-[var(--panel-width-val)] /* Desktop: fixed margins for panels */
                       z-20 p-2 sm:p-4`}>
        <ChatHeader
          languages={languages}
          selectedLanguage={selectedLanguage}
          ...
          // Removed toggleSidebar prop from ChatHeader as SheetTrigger is now in App.js
