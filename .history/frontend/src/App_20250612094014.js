...
import RoastModal from './components/RoastModal.js'; // NEW import
import UnsolicitedAdviceToast from './components/UnsolicitedAdviceToast.js'; // NEW import
...
export default function App() {
  ...
  const [isRoastModalOpen, setIsRoastModalOpen] = useState(false); // New state for Roast Modal
  const [currentRoast, setCurrentRoast] = useState(null); // New state for roast text
  const [isLoadingRoast, setIsLoadingRoast] = useState(false); // New state for roast loading
  const [roastError, setRoastError] = useState(null); // New state for roast error

  const [unsolicitedAdvice, setUnsolicitedAdvice] = useState(null); // New state for unsolicited advice
  ...
  }, [displayMessageBox]); // Memoize and add displayMessageBox as dependency
  ...
  }, [selectedLanguage]); // Memoize and add selectedLanguage as dependency

  // New: Fetch Unsolicited Advice
  const fetchUnsolicitedAdvice = useCallback(async () => {
    try {
      ...
      // Don't set error state, just log for this background feature
    }
  }, [selectedLanguage]); // Memoize and add selectedLanguage as dependency
  ...
  useEffect(() => {
    fetchHumor();
    // Fetch unsolicited advice periodically
    const adviceInterval = setInterval(() => {
        fetchUnsolicitedAdvice();
    }, 30000); // Fetch new advice every 30 seconds

    return () => clearInterval(adviceInterval); // Clear interval on unmount
  ...
  // New: Function to generate a roast
  const generateRoast = useCallback(async () => {
    setIsRoastModalOpen(true); // Open roast modal
    setIsLoadingRoast(true);
    setCurrentRoast(null);
    setRoastError(null);

    try {
      const response = await fetch('http://localhost:5002/roast_me', { // NEW endpoint
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ language: selectedLanguage }), // Send current language
      });
      ...
      const data = await response.json();
      setCurrentRoast(data.roast); // Assuming backend returns {'roast': 'text'}
    } catch (error) {
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <Sidebar
        ...
        onGenerateRoast={generateRoast} // Pass roast generation function to Sidebar
      />
