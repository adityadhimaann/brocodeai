...
// Import all custom components with explicit .js extensions
...
import { Loader2 } from 'lucide-react'; // This import is fine here
...
export default function App() {
  ...
  }, [displayMessageBox]); // FIX: Removed 'loggedInUser' and other variables from dependencies. Only displayMessageBox is a stable dependency here.
  ...
  const handleLogin = useCallback((username, password) => {
    ...
    if (auth) {
        ...
        // FIX: Ensure initializeApp is called only if firebaseApp is not yet set AND a default app isn't already there.
        // This is a common pattern to prevent re-initialization errors in React's StrictMode.
        let app = firebaseApp;
        if (!app && firebase.getApps().length === 0) { // Check if no default app exists
          app = initializeApp(firebaseConfig);
          setFirebaseApp(app); // Store the app instance
        }
        if (app) { // Proceed only if app is valid
            const authInstance = getAuth(app);
            ...
            console.error("Firebase app instance not available for direct anonymous sign-in.");
