...
export default function App() {
  ...
  const [selectedVoiceStyle, setSelectedVoiceStyle] = useState('Default'); // Add this line
  // FIX: Renamed selectedVoiceStyle to selectedPersonaMode for clarity
  const [selectedPersonaMode, setSelectedPersonaMode] = useState('Default brocodeAI'); // Default persona mode
    ...
    try {
      const response = await fetch('http://localhost:5002/chat', {
        ...
        body: JSON.stringify({
          ...
          persona_mode: selectedPersonaMode, // Pass selectedPersonaMode to backend
  ...
  return (
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <div className="hidden md:flex fixed inset-y-0 left-0 w-64 z-40 flex-col py-6 px-4 bg-zinc-900 border-r-4 border-pink-700 shadow-2xl">
        <Sidebar
          toggleSidebar={() => {}} // No-op for desktop
          onSelectChat={handleSelectChat}
          clearMessages={clearMessages}
          displayMessageBox={displayMessageBox}
        />
