...
export default function App() {
  ...
  // FIX: Separate humor states for left and right panels
  ...
  // FIX: fetchHumor now populates two separate states
  const fetchHumor = useCallback(async () => {
    setIsLoadingHumor(true);
    setHumorError(null);
    try {
      // Fetch humor twice or fetch a larger set and split/randomize
      const response1 = await fetch('http://localhost:5002/get_humor', {
        method: 'POST',
      ...
      const response2 = await fetch('http://localhost:5002/get_humor', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ language: selectedLanguage }),
      });
      ...
      if (!response1.ok || !response2.ok) {
        throw new Error((data1.error || data2.error || 'Failed to fetch humor data.'));
      }
      ...
      setHumorItemsLeft(data1);
      setHumorItemsRight(data2);
    ...
    try {
      const response = await fetch('http://localhost:5002/chat', {
        ...
        body: JSON.stringify({
          ...
          voice_style: selectedPersonaMode, // FIX: Use selectedPersonaMode as the voice_style in backend
          persona_mode: selectedPersonaMode, // Also pass persona_mode explicitly if backend needs it separately
  ...
  const speakArbitraryText = useCallback(async (text, lang, personaMode) => { // Use personaMode here
    if (!text) return;
    try {
      console.log(`[App.js] Requesting audio for text: "${text.substring(0, 30)}...", lang: ${lang}, personaMode: ${personaMode}`); // Log personaMode
      const response = await fetch('http://localhost:5002/speak_text', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ text: text, language: lang, voice_style: personaMode }), // Pass personaMode as voice_style
      });
  ...
  return (
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <div className={`hidden md:block fixed inset-y-0 left-0 w-[var(--panel-width-val)] z-10 top-[var(--header-height)] bottom-0`}>
        <ScrollingHumorPanel
            humorItems={humorItemsLeft} // FIX: Use humorItemsLeft
            direction="down"
            position="left"
            ...
            selectedVoiceStyle={selectedPersonaMode} // FIX: Pass selectedPersonaMode
        />
      </div>
      ...
      <div className={`hidden md:block fixed inset-y-0 right-0 w-[var(--panel-width-val)] z-10 top-[var(--header-height)] bottom-0`}>
        <ScrollingHumorPanel
            humorItems={humorItemsRight} // FIX: Use humorItemsRight
            direction="up"
            position="right"
            isLoading={isLoadingHumor}
            error={humorError}
      ...
      <div className="flex flex-col flex-1 relative z-20 
        ...
        <ChatHeader
          ...
          selectedVoiceStyle={selectedPersonaMode} // FIX: Pass selectedPersonaMode as selectedVoiceStyle
          onSelectVoiceStyle={setSelectedPersonaMode} // Pass setSelectedPersonaMode setter
          selectedPersonaMode={selectedPersonaMode} // FIX: Ensure this is correctly passed
          onSelectPersonalityMode={setSelectedPersonaMode} // FIX: Pass the setter for persona mode
