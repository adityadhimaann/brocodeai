...
export default function App() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (isAutoSpeakEnabled && botAudioBase64) {
        ...
        audio.play().catch(e => {
            console.error("Error playing auto-speak audio (likely autoplay blocked or invalid audio):", e);
  ...
  const playReceivedAudio = useCallback((base64Audio) => {
    if (base64Audio) {
      ...
      audio.play().catch(e => {
          console.error("Error playing manual audio (likely autoplay blocked or invalid audio):", e);
  ...
  const speakArbitraryText = useCallback(async (text, lang, voiceStyle) => {
    ...
    try {
      ...
      if (result.audio) {
        ...
        audio.play().catch(e => {
            console.error("Error playing arbitrary audio (likely autoplay blocked or invalid audio):", e);
            displayMessageBox("Browser blocked audio. Click 'Listen' or enable auto-play for this site.", "error");
        });
      } else {
  ...
  // NEW: Function to toggle auto-speak state
