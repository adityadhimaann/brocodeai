...
import Sidebar from './components/Sidebar.js'; // This will be the CONTENT of the SheetContent
...
// NEW: Import Sheet and SheetContent for use directly in App.js
...
export default function App() {
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); // Controls desktop sidebar visibility
  ...
  const handleLogin = useCallback((username, password) => {
    ...
    // Check if Firebase app instance is already initialized
    let app = firebaseApp; // Get the app instance from state
    if (!app) {
        // If no app instance is set in state, try to get the default app that might have been initialized
        if (getApps().length === 0) { // Check if NO Firebase apps are initialized yet
            app = initializeApp(firebaseConfig);
            setFirebaseApp(app); // Store the new app instance in state
        } else {
            app = getApp(); // Get the default app if already initialized (e.g., by onAuthStateChanged)
            setFirebaseApp(app); // Store the app instance in state
        }
    }

    if (app) { // Proceed only if a valid app instance is available
        const authInstance = getAuth(app); // Get auth instance from this app instance
        // FIX: Removed __initial_auth_token from here, as handleLogin is a user-initiated login, not custom token.
        signInAnonymously(authInstance) // Simplified to direct anonymous sign-in for testing
            .then(userCredential => {
                const user = userCredential.user;
                console.log("Firebase Auth: Anonymous sign-in successful from handleLogin:", user.uid);
                // onAuthStateChanged listener in useEffect will pick this up and set userId/loggedInUser
  ...
  }, [displayMessageBox, firebaseApp]); // No longer depends on 'auth' directly here, as 'auth' is derived from 'app'
  ...
  // Removed unused panelWidth, sidebarWidth, mainContentMx
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      {/* Sidebar Component */}
      {/* Sheet component now wraps the Sidebar, controlled by App.js's isSidebarOpen */}
      <Sheet open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
        <SheetContent
          side="left"
          className="w-64 bg-zinc-900 border-r-4 border-pink-700 shadow-2xl z-40 flex flex-col py-6 px-4
                     md:w-64 md:static md:translate-x-0 md:flex" // For desktop: fixed and visible
        >
          {/* Sidebar content moved here from Sidebar.js, passing needed props */}
          <Sidebar
            // isOpen is now managed by Sheet's 'open' prop, so don't pass it down
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out
                       ${isSidebarOpen ? 'ml-64' : 'ml-0'} /* Dynamic ml for sidebar on mobile */
                       md:ml-[var(--panel-width-val)] md:mr-[var(--panel-width-val)] /* Fixed mx for side panels on desktop */
                       z-20 p-2 sm:p-4`}>
        <ChatHeader
          languages={languages}
          selectedLanguage={selectedLanguage}
          ...
          toggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} // This will now control Sheet's open prop
