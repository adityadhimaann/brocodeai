...
export default function App() {
  ...
  // --- NEW/UPDATED: Robust Audio Playback Function ---
  // This function creates a temporary <audio> element and appends it to the DOM for playback.
  const playAudioFromBase64 = useCallback(async (base64Audio, source = "chat") => {
    ...
    audioElement.volume = 1.0; // Ensure volume is not muted
    // Append to body (or a hidden div) so it's in the DOM
    ...
    try {
        ...
        // Remove the audio element after it finishes playing or errors, after a short delay
        // Delay removal slightly to ensure audio has time to play for short clips
        setTimeout(() => {
            if (audioElement.parentNode) {
                audioElement.parentNode.removeChild(audioElement);
                console.log(`[playAudioFromBase64] Audio element removed from DOM for ${source}.`);
            }
        }, 3000); // Remove after 3 seconds, or listen to 'ended' event
    }
  }, [displayMessageBox]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (isAutoSpeakEnabled && botAudioBase64) {
        await playAudioFromBase64(botAudioBase64, "auto-speak chat"); // Use the new helper
  ...
  // playReceivedAudio is now deprecated as playAudioFromBase64 is central
  const playReceivedAudio = useCallback(async (base64Audio) => {
    await playAudioFromBase64(base64Audio, "manual chat");
  }, [playAudioFromBase64]);


  // speakArbitraryText also uses the new centralized player
  const speakArbitraryText = useCallback(async (text, lang, voiceStyle) => {
    ...
    try {
      ...
      if (result.audio) {
        await playAudioFromBase64(result.audio, "humor panel"); // Use the new helper
      } else {
