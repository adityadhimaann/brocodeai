...
export default function App() {
  ...
  const [isAutoSpeakEnabled, setIsAutoSpeakEnabled] = useState(true); // NEW: Auto-speak is ON by default
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // NEW: Auto-speak bot responses if enabled
      if (isAutoSpeakEnabled && botAudioBase64) {
        const audio = new Audio(`data:audio/mp3;base64,${botAudioBase64}`);
        audio.play().catch(e => console.error("Error playing auto-speak audio:", e));
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out
        ...
        <ChatHeader
          ...
          isAutoSpeakEnabled={isAutoSpeakEnabled} // Pass auto-speak state
          onToggleAutoSpeak={toggleAutoSpeak} // Pass auto-speak toggle function
        />
