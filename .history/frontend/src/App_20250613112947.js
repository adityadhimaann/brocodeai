...
import Sidebar from './components/Sidebar.js'; // This will be the content, not the Sheet itself
...
// Shadcn Sheet components for mobile overlay
...
const DUMMY_FIREBASE_CONFIG = {
  apiKey: "YOUR_FIREBASE_API_KEY_HERE", // REPLACE WITH YOUR REAL KEY
...
export default function App() {
  ...
  const handleLogin = useCallback((username, password) => {
    ...
    if (!app) {
        ...
        if (getApps().length === 0) { // Only initialize if no default app exists
    ...
    if (app) { // Proceed only if a valid app instance is available
        ...
        // FIX: Ensure auth state listener is already set up in primary useEffect for user handling
        // This specific login flow should focus on signing in
        signInAnonymously(authInstance) // Simplified to direct anonymous sign-in for testing
            .then(userCredential => {
                ...
                // The onAuthStateChanged listener in useEffect will pick this up and set userId/loggedInUser
                // No need to set userId/loggedInUser directly here to avoid conflicts
  ...
  }, [displayMessageBox, firebaseApp]); // Only depends on displayMessageBox and firebaseApp
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out
                       ${isSidebarOpen ? 'md:ml-64' : 'ml-0'} /* Dynamic ml for sidebar */
                       md:ml-[var(--panel-width-val)] md:mr-[var(--panel-width-val)] /* Fixed mx for side panels */
                       z-20 p-2 sm:p-4`}>
        <ChatHeader
          languages={languages}
          selectedLanguage={selectedLanguage}
