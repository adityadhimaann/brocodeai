...
import BrocodeMemeModal from './components/BrocodeMemeModal.js'; // NEW import for meme modal
...
export default function App() {
  ...
  const [isMemeModalOpen, setIsMemeModalOpen] = useState(false); // State for meme modal visibility
  const [currentMeme, setCurrentMeme] = useState(null); // State for generated meme data
  const [isLoadingMeme, setIsLoadingMeme] = useState(false); // State for meme generation loading
  const [memeError, setMemeError] = useState(null); // State for meme generation error


  const recognitionRef = useRef(null);

  const languages = [
    { name: 'Hinglish', code: 'hinglish' }, // Default and first
  ...
  const generateBrocodeMeme = async () => {
    setIsMemeModalOpen(true); // Open modal immediately
    ...
    try {
      const response = await fetch('http://localhost:5002/generate_brocode_meme', {
        ...
        body: JSON.stringify({ language: selectedLanguage }), // Send current language for meme
      });
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out ${mainContentMl} ${isSidebarOpen ? '' : 'md:ml-80'} md:mr-80 z-20`}>
        ...
        <div className="flex justify-center p-4 pt-0">
            <MessageInput
                ...
                onGenerateBrocodeMeme={generateBrocodeMeme} // Pass the new function
            />
        </div>
      </div>
