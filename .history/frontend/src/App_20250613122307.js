...
import { Loader2, Menu } from 'lucide-react'; // Menu icon for the SheetTrigger
...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      {/* Main Content Area */}
      {/* Use Tailwind's grid system for robust responsive layout */}
      <div className="flex flex-1 flex-col md:grid md:grid-cols-[var(--panel-width-val)_1fr_var(--panel-width-val)] transition-all duration-300 ease-in-out">
        {/* Left Panel (Hidden on Mobile, Grid Item on Desktop) */}
        ...
        {/* Central Content Area (Chat Header, Chat, Input) */}
        <div className="col-span-1 flex flex-col w-full min-w-0"> {/* min-w-0 important for flex/grid */}
          <ChatHeader
            languages={languages}
            selectedLanguage={selectedLanguage}
        ...
        {/* Right Panel (Hidden on Mobile, Grid Item on Desktop) */}
        <div className="hidden md:block col-span-1">
            <ScrollingHumorPanel
                humorItems={humorItems}
                direction="up"
