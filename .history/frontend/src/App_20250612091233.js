...
import ScrollingHumorPanel from './components/ScrollingHumorPanel.js'; // NEW import for scrolling panels
...
export default function App() {
  ...
  const [humorItems, setHumorItems] = useState([]); // State for jokes/memes
  ...
  // Function to fetch humor from backend
  ...
  // Fetch humor on initial load and when language changes
  useEffect(() => {
    fetchHumor();
  }, [selectedLanguage]); // Depend on selectedLanguage to refetch when changed
  ...
  // Determine main content area margins based on sidebar and panel widths
  const panelWidth = 'w-80'; // Corresponds to w-80 in ScrollingHumorPanel
  const sidebarWidth = 'w-64'; // Corresponds to w-64 in Sidebar
  const mainContentMl = isSidebarOpen ? 'md:ml-64' : 'ml-0'; // Sidebar pushes content
  const mainContentMx = 'md:mx-80'; // Panels occupy 80px on each side

  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative"> {/* Added relative for absolute children */}
      {systemMessage && (
        <MessageBox
      ...
      {/* Main Content Area */}
      {/* Adjusted margin to account for fixed side panels */}
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out ${mainContentMl} ${mainContentMx} z-20`}>
        <ChatHeader
          languages={languages}
