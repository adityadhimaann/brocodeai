...
export default function ScrollingHumorPanel({ humorItems = [], direction = 'up', position = 'left', isLoading, error, onRefresh, onSpeakText, selectedLanguage, selectedVoiceStyle }) {
  ...
  const handleListenClick = async (content, itemId) => { // Marked as async
    ...
    try {
      await onSpeakText(content, selectedLanguage, selectedVoiceStyle); // Await the function call
      ...
      // Clear speaking ID immediately after the async call completes,
            ...
            {displayItems.map((item, index) => (
              ...
              >
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  disabled={currentlySpeakingId !== null} // Disable all listen buttons if any item is speaking
                >
