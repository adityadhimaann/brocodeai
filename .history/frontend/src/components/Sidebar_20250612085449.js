...
import { Settings, History, Plus, X, Menu, Search, MoreVertical, LogOut, User } from 'lucide-react';

export default function Sidebar({ isOpen, toggleSidebar, onSelectChat }) {
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex justify-between items-center mb-6 border-b pb-4 border-zinc-700">
            <h2 className="text-2xl font-bold text-teal-400">brocodeAI // Console</h2>
            <motion.button
              onClick={toggleSidebar}
              className="p-2 rounded-full bg-pink-700 hover:bg-pink-600 text-white focus:outline-none focus:ring-2 focus:ring-pink-500"
          ...
          {/* New Chat Button */}
          <motion.button
            className="w-full py-3 mb-6 bg-teal-600 hover:bg-teal-500 text-white text-lg font-bold rounded-lg shadow-lg flex items-center justify-center space-x-2 focus:outline-none focus:ring-2 focus:ring-teal-500"
          ...
          <div className="flex items-center text-zinc-300 font-semibold mb-4 space-x-2">
            ...
            <div className="relative ml-auto">
              <motion.button
                ...
                className="p-1 rounded-full hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-zinc-600"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
              ...
              <AnimatePresence>
                {showHistoryOptions && (
                  ...
                  >
                    <button className="block w-full text-left px-4 py-2 text-sm text-zinc-200 hover:bg-zinc-600">
                      <Search className="inline-block h-4 w-4 mr-2" /> Search History
                    </button>
                    <button className="block w-full text-left px-4 py-2 text-sm text-zinc-200 hover:bg-zinc-600">
                      <History className="inline-block h-4 w-4 mr-2" /> All Interactions
                    </button>
                    <button className="block w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-zinc-600">
          ...
          <div className="mt-6 border-t pt-4 border-zinc-700">
            <motion.button
              ...
              className="w-full py-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-semibold rounded-lg flex items-center justify-center space-x-2 focus:outline-none focus:ring-2 focus:ring-zinc-600 mb-3"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
            ...
            <motion.button
              className="w-full py-3 bg-red-800 hover:bg-red-700 text-white font-semibold rounded-lg flex items-center justify-center space-x-2 focus:outline-none focus:ring-2 focus:ring-red-600"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
