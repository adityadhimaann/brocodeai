...
// FIX: Change to a default exported function for consistent import
export default function ScrollingHumorPanel({
  ...
  error,
  onRefresh,
  onSpeakText, // This will be playAudioFromBase64 passed from Humor.js
  selectedLanguage,
  selectedPersonaMode, // FIX: Use selectedPersonaMode consistently here
  displayMessageBox
}) {
  const [selectedItem, setSelectedItem] = useState(null);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const controls = useAnimation();
  const containerRef = useRef(null); // Ref for the outer scrolling container (overflow-y-scroll)
  ...
  useEffect(() => {
    const startAnimation = async () => {
      if (!isPaused && !isManuallyScrolling && humorItems.length > 0 && containerRef.current && contentRef.current) {
        const contentHeight = contentRef.current.scrollHeight; // Actual height of the entire content
        const visibleHeight = containerRef.current.offsetHeight; // Visible area height

        // Only start animation if content overflows and is large enough
        if (contentHeight <= visibleHeight || contentHeight < minContentHeightForScroll) {
          controls.stop();
          return;
        }

        // Calculate a duration that scales with content size but maintains a consistent speed
        // We duplicate items 3 times, so we scroll 2/3 of the total duplicated content height for seamless loop.
        const distanceToScroll = contentHeight / 3; // Scroll one full set of original items to loop
  ...
  }, [humorItems, direction, isPaused, isManuallyScrolling, controls, speed]); // Added speed to dependencies
  ...
  }, [humorItems, controls]); // Added controls to dependencies
  ...
  return (
    <div
      className={`relative bg-gradient-to-b from-zinc-900 to-zinc-800 h-full flex flex-col border-2 
                  ${position === 'left' ? 'border-r-4 border-pink-700/80' : 'border-l-4 border-pink-700/80'} 
                  shadow-lg shadow-pink-700/20 z-30 overflow-hidden rounded-xl /* Added rounded-xl */
                  /* Removed individual border styles from here, will manage from parent App.js wrapper for panels */
                  `}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
    >
      <div className="flex justify-between items-center p-2 bg-gradient-to-r from-pink-700 to-purple-700 border-b-2 border-pink-500 sticky top-0 z-50 flex-shrink-0 rounded-t-xl"> {/* Added rounded-t-xl */}
