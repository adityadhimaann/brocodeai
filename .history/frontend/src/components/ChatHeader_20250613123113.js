...
export default function ChatHeader({
  ...
  toggleSidebar, // This prop now correctly toggles the isSidebarOpen state in App.js directly
  ...
  return (
    <motion.header
      ...
      className="flex flex-row flex-wrap items-center justify-between gap-x-2 gap-y-2 px-3 py-3 md:px-4 md:py-4 bg-zinc-800 shadow-2xl rounded-b-xl border-b-4 border-teal-600 z-20"
    >
      {/* Left Block: Menu Button (mobile only), Roast, Achievement */}
      <div className="flex items-center gap-x-2 flex-shrink-0"> {/* flex-shrink-0 prevents shrinking */}
        <motion.button
          onClick={toggleSidebar} // This directly calls the setter in App.js to open the Sheet
        ...
        >
          ...
          <span className="hidden sm:inline">Achieve!</span> {/* Shortened text for small screens */}
        </motion.button>
      </div>


      {/* Center Block: brocodeAI Branding */}
      <div className="flex items-center gap-x-1 md:gap-x-3 mx-auto flex-shrink-0"> {/* mx-auto centers it, flex-shrink-0 prevents squeezing */}
        <img
        ...
        />
        <h1 className="text-xl md:text-3xl font-extrabold text-teal-400 drop-shadow-lg tracking-wide whitespace-nowrap"> {/* whitespace-nowrap to prevent line breaks */}
          brocodeAI
        </h1>
      </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <span className="truncate max-w-[40px] sm:max-w-[60px] md:max-w-[80px]"> {/* Adjusted max-w */}
