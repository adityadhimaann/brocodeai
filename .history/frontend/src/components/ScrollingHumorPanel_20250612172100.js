...
export default function ScrollingHumorPanel({ humorItems = [], direction = 'up', position = 'left', isLoading, error, onRefresh, onSpeakText, selectedLanguage, selectedVoiceStyle }) {
  ...
  const [currentlySpeakingId, setCurrentlySpeakingId] = useState(null); // Track which item is speaking
  ...
  const handleListenClick = async (content, itemId) => {
    if (currentlySpeakingId === itemId) {
      // If already speaking this item, stop it (conceptual: stop previous audio)
      // For this example, we'll just return, as stopping audio requires more complex control.
      return;
    }
    
    setCurrentlySpeakingId(itemId); // Mark this item as currently speaking
    try {
      // Call the onSpeakText prop which connects to the backend
      await onSpeakText(content, selectedLanguage, selectedVoiceStyle);
    } catch (err) {
      console.error("Failed to play humor audio:", err);
    } finally {
            ...
            {displayItems.map((item, index) => (
              ...
              >
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  disabled={currentlySpeakingId === `${item.type}-${index}`} // Disable if currently speaking
                >
