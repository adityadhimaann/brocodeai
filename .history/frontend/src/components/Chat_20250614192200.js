...
export default function Chat({
  ...
  // Helper to fetch a Google Images result for a meme caption
  async function fetchGoogleImageForMeme(query) {
    // This is a placeholder. In production, use a backend proxy or a safe image API.
    // For demo, use Unsplash as a free image source for the query.
    // You can replace this with a real Google Images API or similar.
    const url = `https://source.unsplash.com/600x400/?${encodeURIComponent(query + ', meme, bollywood, funny')}`;
    return url;
  }
  ...
  const generateBrocodeMeme = async () => {
    ...
    try {
      ...
      // data.meme or data.caption should be the meme text
      const caption = data.meme || data.caption || 'Desi Meme';
      const image_url = await fetchGoogleImageForMeme(caption);
