...
// Replace MessageSquareMore with MessageSquare
...
export default function ChatHeader({
  ...
  const personalityModes = [
    { name: 'Default brocodeAI', mode: 'default' },
    { name: 'Taau with Trust Issues', mode: 'taau' },
    { name: 'Pados wali Aunty', mode: 'aunty' },
    { name: 'Delhi Dude (Ranveer)', mode: 'ranveer' },
    { name: 'School ke Topper ka Toxic Ghost', mode: 'topper' },
  ];

  const handleDummyLogin = () => {
    const username = prompt("Input Username (Hint: 'bro')");
    const password = prompt("Enter password (Hint: 'code')");
  ...
  return (
    <motion.header
      ...
      className="flex w-full flex-row items-center justify-between gap-x-2 gap-y-2 px-3 py-3 md:px-6 py-4 md:p-4 bg-zinc-900 border-b-4 border-teal-600 shadow-lg rounded-t-xl z-20"
    >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <span className="truncate max-w-[40px] sm:max-w-[60px] md:max-w-[80px]">
                {personalityModes.find(p => p.mode === selectedPersonaMode)?.name || 'Mode'}
              </span>
          ...
          <DropdownMenuContent className="w-48 md:w-56 bg-zinc-700 rounded-lg shadow-2xl border border-zinc-600 overflow-hidden">
            ...
            {personalityModes.map((mode) => (
              <DropdownMenuItem
                key={mode.mode}
                onSelect={() => onSelectPersonalityMode(mode.mode)}
            ...
            >
              {loggedInUser ? (
                <img
                  ...
                  alt="User Profile"
                  className="h-5 w-5 md:h-6 md:w-6 rounded-full border-2 border-teal-400 object-cover"
                />
              ) : (
