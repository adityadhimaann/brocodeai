...
const ScrollingHumorPanel = ({
  ...
  return (
    <div
      className={`relative h-full overflow-y-auto flex flex-col bg-gradient-to-b from-zinc-900 to-zinc-800 border-2 shadow-xl ${
        position === 'left'
          ? 'border-r-4 border-pink-700/80'
          : 'border-l-4 border-pink-700/80'
      }`}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      onScroll={handleScroll}
    >
      <div className="flex justify-between items-center p-3 bg-gradient-to-r from-pink-700 to-purple-700 border-b-2 border-pink-500 sticky top-0 z-50">
      ...
      {error && (
        <div className="p-4 text-red-400 text-center font-semibold">
          Error: {error} 🤦‍♂️
        </div>
      )}
        ...
        <div className="flex-1 overflow-hidden" ref={containerRef}>
          <motion.div animate={controls} className="flex flex-col gap-4 p-4">
            {[...humorItems, ...humorItems].map((item, index) => (
              <motion.div
                key={`${item.id}-${index}`}
                className="relative bg-zinc-800/80 backdrop-blur-sm rounded-lg p-4 cursor-pointer border border-pink-500/30 hover:border-pink-400 transition-all duration-300 shadow hover:shadow-pink-500/20"
                onClick={(e) => handleItemClick(e, item)}
                whileHover={{ scale: 1.015 }}
              >
                <div className="absolute top-2 right-2 bg-pink-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow-sm">
                ...
                {item.type === 'meme' && item.image_url && (
                  <img
                    src={item.image_url}
                    alt="Meme"
                    className="w-full max-h-80 object-cover rounded-md mb-3 border border-zinc-700"
                    style={{ pointerEvents: 'none' }}
                  />
                )}
                <p className="text-zinc-100 text-sm font-medium leading-relaxed">{item.content}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <Dialog open={!!selectedItem} onOpenChange={() => setSelectedItem(null)}>
        <DialogContent className="bg-zinc-900 text-zinc-100 border border-pink-700 max-w-md">
          <DialogHeader>
            <DialogTitle>
              {selectedItem?.type === 'meme' ? 'Meme' : 'Joke'}
            </DialogTitle>
          ...
          {selectedItem?.type === 'meme' && selectedItem?.image_url && (
            <img
              ...
              alt="Meme"
              className="w-full max-h-80 object-contain rounded-md my-2"
            />
          )}
          <DialogFooter className="flex flex-wrap gap-2 pt-4">
            <Button
              ...
              disabled={isSpeaking}
              className="border-pink-700 text-white hover:bg-pink-700 flex gap-1"
            >
              {isSpeaking ? (
