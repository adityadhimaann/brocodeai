...
export default function ScrollingHumorPanel({ humorItems = [], direction = 'up', position = 'left', isLoading, error, onRefresh, onSpeakText, selectedLanguage, selectedVoiceStyle }) {
  ...
  useEffect(() => {
    const calculateScrollDuration = () => {
      if (contentRef.current && panelRef.current && displayItems.length > 0) {
        ...
        const singleSetHeight = contentRef.current.scrollHeight / 5; // Divide by duplication factor
        const totalContentHeight = contentRef.current.scrollHeight; // Total height of duplicated content

        console.log(`[ScrollPanel Debug] Panel Height: ${panelHeight}, Single Set Height: ${singleSetHeight}, Total Content Height: ${totalContentHeight}`);

        if (totalContentHeight > panelHeight * 1.5) { // Only scroll if content is significantly taller than the panel
                                                      // Multiply by 1.5 to ensure there's enough content to scroll without blank space
          // Calculate duration: Longer content = longer duration for same speed.
          // Adjust '50' for desired speed (lower = faster, higher = slower)
          const duration = totalContentHeight / 25; // Roughly pixels per second (adjust 25 for desired speed)
          setScrollDuration(`${Math.max(30, duration)}s`); // Ensure a minimum duration (e.g., 30s)
          setIsReadyToScroll(true);
          console.log(`[ScrollPanel Debug] Calculated scroll duration: ${Math.max(30, duration)}s`);
        } else {
          setScrollDuration('0s'); // No scrolling if content fits or is too short
          setIsReadyToScroll(false);
          console.log("[ScrollPanel Debug] Content fits or too short for continuous scroll, animation disabled.");
        }
      } else {
    ...
    // Recalculate on initial render, and on window resize
    calculateScrollDuration();
    window.addEventListener('resize', calculateScrollDuration);

    // Also trigger recalculation after a short delay to ensure content is rendered
    const timeoutId = setTimeout(calculateScrollDuration, 500); // Give time for content to render
  ...
  const handleListenClick = async (content, itemId) => {
    ...
    try {
      ...
      // Clear speaking state immediately after the async call completes,
      // as the actual audio playback is handled within onSpeakText/App.js
