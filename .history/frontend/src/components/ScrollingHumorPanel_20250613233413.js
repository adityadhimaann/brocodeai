...
import { RefreshCw, Volume2, Share2, Copy, Loader2, Link } from 'lucide-react'; // Added Link for direct URL if needed
...
const ScrollingHumorPanel = ({
  ...
  onSpeakText,
  selectedLanguage,
  selectedPersonaMode, // FIX: Use selectedPersonaMode consistently
  displayMessageBox
}) => {
  ...
  const contentRef = useRef(null); // Ref for the inner content div (motion.div)
  const [isPaused, setIsPaused] = useState(false);
  const [isManuallyScrolling, setIsManuallyScrolling] = useState(false);
  const scrollTimeoutRef = useRef(null);

  const speed = 100; // Adjust for desired items per second, was 80
  const itemHeightEstimate = 150; // Estimate average item height to calculate initial duration (adjust based on actual content)
  ...
  useEffect(() => {
    // This effect controls the continuous animation
    const startAnimation = async () => {
      if (!isPaused && !isManuallyScrolling && humorItems.length > 0 && containerRef.current && contentRef.current) {
        const contentHeight = contentRef.current.offsetHeight; // Actual height of the entire content
        const containerHeight = containerRef.current.offsetHeight; // Visible area height
        
        // If content doesn't fill the container, no need to scroll
        if (contentHeight <= containerHeight) {
            controls.stop();
            return;
        }

        // Calculate duration based on total content length, ensuring a seamless loop.
        // We duplicate items 3 times, so we scroll 2/3 of the total duplicated content height.
        // The animation then loops when it reaches the end of the *first* duplication.
        const totalDuplicatedContentHeight = contentHeight; // This is the height of all 3 sets of items
        const scrollDistance = totalDuplicatedContentHeight / 3 * 2; // Scroll two full sets to loop seamlessly

        // Duration is proportional to the scroll distance
        const duration = (scrollDistance / (speed * 2)); // Adjust speed multiplier for faster/slower overall animation


        let fromY = direction === 'up' ? 0 : -scrollDistance;
        let toY = direction === 'up' ? -scrollDistance : 0;
        
        // Ensure starting position is always top for 'down' and bottom for 'up'
        // This makes the loop visually seamless
        if (direction === 'down') {
            fromY = -contentHeight / 3; // Start from the beginning of the second set of items
            toY = 0; // Scroll to the end of the first set (which is top of initial content)
        } else { // 'up'
            fromY = 0; // Start from top of first set
            toY = -contentHeight / 3; // Scroll to top of second set
        }


        // Set initial position (instant) before starting continuous animation
    ...
    // Recalculate and start animation whenever humorItems, direction, or pause state changes
    startAnimation();
  }, [humorItems, direction, controls, isPaused, isManuallyScrolling, speed]);


  useEffect(() => {
    // Reset scroll position when items change to prevent jumps if items re-render
  ...
  const handleScroll = () => {
    ...
    controls.stop(); // Stop auto-scroll when user manually scrolls
    ...
    scrollTimeoutRef.current = setTimeout(() => {
      ...
      // Animation will restart due to useEffect dependency when isManuallyScrolling changes
  ...
  const handleSpeak = async (content) => {
    setIsSpeaking(true);
    try {
      await onSpeakText(content, selectedLanguage, selectedPersonaMode); // FIX: Use selectedPersonaMode
  ...
  return (
    <div
      className={`relative bg-gradient-to-b from-zinc-900 to-zinc-800 h-full overflow-hidden flex flex-col border-2 ${position === 'left' ? 'border-r-4 border-pink-700/80' : 'border-l-4 border-pink-700/80'} shadow-lg shadow-pink-700/20 z-30`}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      // onScroll removed from here, managed by containerRef
    >
      ...
      {!isLoading && !error && humorItems.length > 0 && (
        <div className="flex-1 overflow-y-scroll custom-scrollbar" ref={containerRef} onScroll={handleScroll}>
          <motion.div animate={controls} className="flex flex-col gap-4 p-4" ref={contentRef}>
            {/* Duplicate content enough times to create a seamless loop */}
            {[...humorItems, ...humorItems, ...humorItems].map((item, index) => (
              <motion.div
                key={`${item.content}-${item.type}-${index}`} // More robust key
                ...
                style={{ zIndex: 10, flexShrink: 0 }} // Added flexShrink: 0
              >
                <div className="absolute top-2 right-2 bg-pink-600/80 text-white text-xs font-bold px-2 py-1 rounded-full whitespace-nowrap">
