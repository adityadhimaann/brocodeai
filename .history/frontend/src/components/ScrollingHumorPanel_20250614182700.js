...
const ScrollingHumorPanel = ({
  ...
  return (
    <div
      className={`relative h-full flex flex-col bg-gradient-to-b from-zinc-900 via-zinc-800 to-zinc-900 overflow-hidden`}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      onScroll={handleScroll}
    >
      <div className="flex justify-between items-center p-4 backdrop-blur-lg bg-zinc-900/40 sticky top-0 z-50">
        <h2 className="text-base font-semibold text-white tracking-tight">
          {position === 'left' ? '🌀 Left Vibes' : '🌀 Right Vibes'}
        </h2>
        <Button
          variant="ghost"
          size="icon"
