...
export default function ScrollingHumorPanel({
  ...
  selectedLanguage,
  selectedVoiceStyle,
  displayMessageBox, // Added prop for user feedback
}) {
    ...
    const calculateScrollDuration = () => {
      if (contentRef.current && panelRef.current && displayItems.length > 0) {
        ...
        if (contentHeight > panelHeight * 1.2) { // Ensure enough content to scroll
          ...
          console.log(`[ScrollPanel] Panel: ${panelHeight}px, Content: ${contentHeight}px, Duration: ${duration}s`);
        } else {
          setScrollDuration(0); // No scrolling if content fits
          console.log('[ScrollPanel] Content too short for scrolling.');
        }
      } else {
        setScrollDuration(0);
        console.log('[ScrollPanel] No content or refs not ready.');
      }
    };
        ...
        <div className="scrolling-container h-full overflow-hidden">
          ...
          >
            {displayItems.map((item, index) => (
              <div
                key={`${item.id || item.type}-${index}`} // Improved key robustness
