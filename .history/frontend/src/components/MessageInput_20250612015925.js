...
export default function MessageInput({ input, setInput, sendMessage, toggleListening, isListening, isLoading }) {
  return (
    <div className="flex-shrink-0 bg-gray-800 p-4 border-t border-gray-700 flex items-center space-x-3 rounded-t-lg">
      <input
        type="text"
        className="flex-1 p-3 rounded-full bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 transition-all duration-200"
        ...
        disabled={isLoading || isListening} // Disable input when loading or listening
      />
      <button
