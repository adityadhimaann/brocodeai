...
export default function ChatHeader({
  ...
  // Removed old voice style props
  toggleSidebar,
  loggedInUser, onLogin, onLogout, onGenerateRoast,
  isAutoSpeakEnabled, onToggleAutoSpeak,
  onUnlockAchievement,
  // FIX: Using new persona mode props
  selectedPersonaMode, // Current selected personality mode
  onSelectPersonalityMode, // Function to set personality mode
}) {
  const [showLanguageSelection, setShowLanguageSelection] = useState(false);
  const [showProfileOptions, setShowProfileOptions] = useState(false);
  // FIX: Renamed state to reflect persona mode selection
  const [showPersonaModeSelection, setShowPersonaModeSelection] = useState(false);

  // FIX: Renamed and expanded personality modes
  const personalityModes = [
    ...
    // Add more modes here as they are implemented in backend logic
    // { name: 'Fake Call Center Voice', mode: 'Fake Call Center Voice Mode' },
    // { name: 'Desi Baba Predictor', mode: 'Desi Baba Predictor' },
    // { name: 'Sarcasm Nahi Samjhega Tu', mode: 'Sarcasm Nahi Samjhega Tu Mode' },
    // { name: 'TV Serial Reactions', mode: 'TV Serial Style Reactions' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-x-2 flex-shrink-0">
        {/* The Menu button now directly calls the toggleSidebar prop from App.js */}
        <motion.button
      ...
      <div className="flex items-center gap-x-2 flex-shrink-0">
        ...
        {/* Personality Mode Selector using Shadcn DropdownMenu */}
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <Volume2 className="h-4 w-4 mr-1" /> {/* Using Volume2 for personality mode icon */}
          ...
          <DropdownMenuContent className="w-48 md:w-56 bg-zinc-700 rounded-lg shadow-2xl border border-zinc-600 overflow-hidden">
            ...
            {personalityModes.map((mode) => (
              <DropdownMenuItem
                key={mode.mode}
                onSelect={() => onSelectPersonalityMode(mode.mode)} {/* Use onSelectPersonalityMode */}
