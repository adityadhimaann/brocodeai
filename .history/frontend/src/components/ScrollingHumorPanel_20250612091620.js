...
import { Laugh, X, AlertTriangle, RefreshCw } from 'lucide-react'; // Added RefreshCw to import
...
export default function ScrollingHumorPanel({ humorItems = [], direction = 'up', position = 'left', isLoading, error, onRefresh }) {
  ...
  useEffect(() => {
    if (panelRef.current && displayItems.length > 0) {
      ...
      // Multiply by 3 because we have duplicated the content 3 times for seamless scrolling
      const singleContentHeight = panelRef.current.scrollHeight / 3;
      // Adjust duration based on content height for consistent speed (pixels per second)
      // The '50' here determines the scrolling speed (lower value = faster scroll)
      const duration = singleContentHeight / 50;
      setScrollDuration(`${Math.max(20, duration)}s`); // Ensure a minimum duration for very short content
    }
  ...
  const panelBg = 'bg-zinc-800'; // Background for both panels
