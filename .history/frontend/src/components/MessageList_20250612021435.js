...
import { Volume2 } from 'lucide-react'; // Icon for listen button
...
export default function MessageList({ messages, isLoading, playReceivedAudio }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-zinc-900 border-t border-b border-zinc-700">
      {messages.length === 0 && (
        <motion.div
      ...
      <AnimatePresence>
        {messages.map((msg, index) => (
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            exit="hidden" // For when messages might be removed
            layout // Enable layout animations for smooth shifts
