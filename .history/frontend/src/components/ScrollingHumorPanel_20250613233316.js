...
// FIX: Change to a named function declaration then export default
const ScrollingHumorPanel = ({ // Changed from const to named function
  humorItems,
  direction,
  position,
  isLoading,
  ...
  const containerRef = useRef(null); // Ref for the scrollable container (overflow-y-scroll)
  ...
  const minScrollDuration = 20; // Minimum duration in seconds to prevent very fast scrolls for small content
  ...
  useEffect(() => {
    const startAnimation = async () => {
      if (!isPaused && !isManuallyScrolling && humorItems.length > 0 && containerRef.current && contentRef.current) {
        ...
        const baseDurationPerScreen = 25; // seconds per screen height (adjust for desired speed)
        const effectiveContentHeight = contentHeight / 3;
        const duration = Math.max(minScrollDuration, (effectiveContentHeight / containerHeight) * baseDurationPerScreen);

        let fromY, toY;
        if (direction === 'down') {
            fromY = -contentHeight / 3;
            toY = 0;
        } else { // 'up'
            fromY = 0;
            toY = -contentHeight / 3;
  ...
  const handleShare = async (content, imageUrl) => {
    try {
      if (navigator.share) {
        ...
        displayMessageBox('Share API not supported. Try copy-pasting, it’s not rocket science.', 'error');
      }
    } catch (error) {
  ...
  return (
    <div
      className={`relative bg-gradient-to-b from-zinc-900 to-zinc-800 h-full flex flex-col border-2 ${position === 'left' ? 'border-r-4 border-pink-700/80' : 'border-l-4 border-pink-700/80'} shadow-lg shadow-pink-700/20 z-30`}
        ...
        <div className="flex-1 overflow-y-scroll custom-scrollbar" ref={containerRef} onScroll={handleScroll}>
          <motion.div animate={controls} className="flex flex-col gap-4 p-4" ref={contentRef}>
            ...
            {[...humorItems, ...humorItems, ...humorItems].map((item, index) => (
              <motion.div
                key={`<span class="math-inline">\{item\.content\}\-</span>{item.type}-${index}`} // Robust key for duplicated items
