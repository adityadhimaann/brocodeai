...
export default function Sidebar({ isOpen, toggleSidebar, onSelectChat, clearMessages, displayMessageBox }) {
  ...
  // Function to handle "New Interaction" button click
  ...
  // Function to handle "Clear All History" button click
  const handleClearAllHistory = () => {
    // In a real app, this would involve a backend API call to delete history
    ...
    if (window.confirm("Are you certain you wish to purge all interaction records? This action is irreversible.")) {
      // For demonstration, just clear the current messages
      clearMessages();
      displayMessageBox("Interaction history deleted. My memory banks are now... cleaner. Sort of.", "info");
      setShowHistoryOptions(false); // Close history options
    }
  };

  // Functions for conceptual buttons (will show MessageBox feedback)
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            // Apply fixed positioning always, width, and z-index.
            // On desktop (md:), it will always be visible, but its presence will shift main content.
            // On mobile, it's an overlay.
          ...
          >
            ...
            <div className="flex justify-between items-center mb-6 border-b pb-4 border-zinc-700">
              ...
              <motion.button
                onClick={toggleSidebar}
                className="p-2 rounded-full bg-pink-700 hover:bg-pink-600 text-white focus:outline-none focus:ring-2 focus:ring-pink-500 transition-colors duration-200 md:hidden" {/* Hide close button on desktop (md screens and up) */}
