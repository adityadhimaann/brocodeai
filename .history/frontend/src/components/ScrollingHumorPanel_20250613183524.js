...
export default function ScrollingHumorPanel({ humorItems = [], direction = 'up', position = 'left', isLoading, error, onRefresh, onSpeakText, selectedLanguage, selectedVoiceStyle }) {
  ...
  const [isReadyToScroll, setIsReadyToScroll] = useState(false); // New state to ensure content is measured

  // Duplicate items to create a seamless loop. Ensure at least 2 full sets for smooth loop.
  const displayItems = humorItems.length > 0 ? [...humorItems, ...humorItems, ...humorItems, ...humorItems] : []; // Duplicated more times

  useEffect(() => {
    // Only calculate and start scrolling if content is available and rendered
    if (contentRef.current && panelRef.current && displayItems.length > 0) {
      const panelHeight = panelRef.current.clientHeight;
      const contentHeight = contentRef.current.scrollHeight; // Height of the duplicated content

      console.log(`[ScrollingHumorPanel] Panel Height: ${panelHeight}, Content Height: ${contentHeight}`);

      if (contentHeight > panelHeight) { // Only scroll if content is taller than the panel
        // Calculate duration based on total duplicated content height for seamless loop
        // Adjust the divisor (e.g., 50) for desired speed (lower = faster)
        const duration = contentHeight / 25; // Slower speed for smoother loop
        setScrollDuration(`${Math.max(30, duration)}s`); // Ensure a minimum duration, e.g., 30s
        setIsReadyToScroll(true);
        console.log(`[ScrollingHumorPanel] Calculated scroll duration: ${duration}s`);
      } else {
        setScrollDuration('0s'); // No scrolling if content fits
        setIsReadyToScroll(false);
        console.log("[ScrollingHumorPanel] Content fits, no scrolling needed.");
      }
    } else {
  ...
  const scrollingStyle = {
    ...
    animationPlayState: isHovered || !isReadyToScroll ? 'paused' : 'running', // Pause on hover or if not ready
  };
  ...
  const handleListenClick = async (content, itemId) => {
    ...
    if (currentlySpeakingId !== null) { // If something is already speaking, prevent new audio
    ...
    try {
      ...
      setCurrentlySpeakingId(null); // Clear speaking state after audio finishes or on error
    }
  };

  return (
    <motion.div
      ref={panelRef} // Attach panelRef to the outer div
    ...
    >
      ...
      {isLoading ? (
        ...
        <div
          // ref={panelRef} // Removed panelRef from here as it's on the outer div
          className="w-full h-full flex-1 overflow-hidden"
          style={{ position: 'relative' }}
        >
          <div
            ref={contentRef} // Attach contentRef to the inner scrollable div
            className="w-full absolute inset-0 will-change-transform scrolling-content"
            style={scrollingStyle}
          >
            {displayItems.map((item, index) => (
              <div
                key={`${item.type}-${index}-${item.content.substring(0,10)}`} // More robust key to prevent React errors
