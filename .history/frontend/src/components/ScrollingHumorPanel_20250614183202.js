...
export default function ScrollingHumorPanel({
  ...
  onSpeakText,
  selectedLanguage,
  selectedPersonaMode, // Using selectedPersonaMode
  displayMessageBox
}) => {
  const [selectedItem, setSelectedItem] = useState(null);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const controls = useAnimation();
  const containerRef = useRef(null); // Ref for the div that handles overflow-y-scroll
  const contentRef = useRef(null); // Ref for the inner motion.div content
  const [isPaused, setIsPaused] = useState(false);
  const [isManuallyScrolling, setIsManuallyScrolling] = useState(false);
  const scrollTimeoutRef = useRef(null);

  // Speed controls how fast content scrolls (pixels per second)
  const scrollSpeed = 50; // pixels per second (adjust as needed, was 0.05 px/ms)
  const minContentHeightForScroll = 300; // Minimum actual content height to enable animation
  ...
  useEffect(() => {
    const startAnimation = async () => {
      if (!isPaused && !isManuallyScrolling && humorItems.length > 0 && containerRef.current && contentRef.current) {
        ...
        const distanceToScroll = contentHeight; // Distance to scroll one full set of original items to loop
        const duration = distanceToScroll / scrollSpeed; // Calculate duration based on distance and speed

        let fromY = direction === 'up' ? 0 : -distanceToScroll;
        let toY = direction === 'up' ? -distanceToScroll : 0;
        
        // Reset to a known position before starting animation
    ...
    startAnimation();
    return () => controls.stop(); // Cleanup animation on unmount or dependency change
  }, [humorItems, direction, isPaused, isManuallyScrolling, controls, scrollSpeed]);
  ...
  const handleScroll = () => {
    ...
    scrollTimeoutRef.current = setTimeout(() => {
      ...
      // When manual scroll ends, animation will restart due to useEffect dependencies
  ...
  const handleSpeak = async (content) => {
    setIsSpeaking(true);
    try {
      await onSpeakText(content, selectedLanguage, selectedPersonaMode);
  ...
  return (
    // Outer container: no borders/shadows here, these will be applied by the parent in App.js
    // Added rounded-b-xl and other classes for visual polish
    <div
      className={`relative bg-gradient-to-b from-zinc-900 to-zinc-800 h-full flex flex-col overflow-hidden rounded-xl shadow-lg`}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
    >
      <div className="flex justify-between items-center p-2 bg-gradient-to-r from-pink-700 to-purple-700 border-b-2 border-pink-500 sticky top-0 z-50 flex-shrink-0 rounded-t-xl">
      ...
      {!isLoading && !error && humorItems.length > 0 && (
        <div className="flex-1 overflow-y-scroll custom-scrollbar relative" ref={containerRef} onScroll={handleScroll}>
          <motion.div animate={controls} className="flex flex-col gap-4 p-4 absolute inset-0" ref={contentRef}>
            {/* Duplicate content 3 times for seamless loop */}
            {[...humorItems, ...humorItems, ...humorItems].map((item, index) => (
              <motion.div
                key={`${item.content}-${item.type}-${index}`}
              ...
              >
                <div className="absolute top-2 right-2 bg-pink-600/80 text-white text-xs font-bold px-2 py-1 rounded-full whitespace-nowrap z-20">
      ...
      <Dialog open={!!selectedItem} onOpenChange={() => setSelectedItem(null)}>
        <DialogContent className="bg-zinc-900/90 backdrop-blur-lg text-white border-2 border-pink-700 max-w-md p-6 rounded-xl"> {/* Added rounded-xl */}
          <DialogHeader>
