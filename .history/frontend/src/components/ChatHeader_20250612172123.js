...
import { ChevronDown, MessageSquareMore, Menu, User, LogIn, LogOut, Flame, Volume2 } from 'lucide-react'; // Added Volume2 for voice style icon

export default function ChatHeader({ languages, selectedLanguage, onSelectLanguage, selectedVoiceStyle, onSelectVoiceStyle, toggleSidebar, loggedInUser, onLogin, onLogout, onGenerateRoast }) {
  const [showLanguageSelection, setShowLanguageSelection] = useState(false);
  const [showProfileOptions, setShowProfileOptions] = useState(false);
  const [showVoiceStyleSelection, setShowVoiceStyleSelection] = useState(false); // New state for voice style dropdown
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        ...
        <div className="relative">
          <button
            ...
            aria-expanded={showVoiceStyleSelection}
            aria-haspopup="true"
          >
            ...
            <ChevronDown className={`h-3 w-3 md:h-4 md:w-4 ml-1 md:ml-2 transition-transform duration-200 ${showVoiceStyleSelection ? 'rotate-180' : 'rotate-0'}`} />
          </button>
          <AnimatePresence>
            {showVoiceStyleSelection && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.2 }}
                className="absolute right-0 mt-3 w-40 md:w-48 bg-zinc-700 rounded-lg shadow-2xl z-10 border border-zinc-600 overflow-hidden"
              >
                {voiceStyles.map((voice) => (
                  <motion.button
                    key={voice.style}
                    onClick={() => {
                      onSelectVoiceStyle(voice.style);
                      setShowVoiceStyleSelection(false);
                    }}
                    whileHover={{ backgroundColor: '#4f46e5', color: '#fff' }} // Indigo-600 hover
                    whileTap={{ scale: 0.95 }}
                    className={`block w-full text-left px-4 py-2 text-xs md:text-sm font-medium transition-colors duration-200
                                ${selectedVoiceStyle === voice.style ? 'bg-indigo-600 text-white' : 'text-zinc-200 hover:text-white'}`}
                  >
                    {voice.name}
                  </motion.button>
                ))}
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown className={`h-3 w-3 md:h-4 md:w-4 ml-1 md:ml-2 transition-transform duration-200 ${showLanguageSelection ? 'rotate-180' : 'rotate-0'}`} />
          </button>
          <AnimatePresence>
        ...
        {/* Profile/Login Section (existing) */}
        <div className="relative">
          <motion.button
          ...
          <AnimatePresence>
            {showProfileOptions && (
              ...
              >
                {loggedInUser ? (
                  <>
                    <div className="px-4 py-2 md:px-5 md:py-3 text-xs md:text-sm text-zinc-300 border-b border-zinc-600">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="block w-full text-left px-4 py-2 text-xs md:text-sm font-medium text-red-400 hover:text-white transition-colors duration-200 flex items-center space-x-1 md:space-x-2"
                    >
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="block w-full text-left px-4 py-2 text-xs md:text-sm font-medium text-teal-300 hover:text-white transition-colors duration-200 flex items-center space-x-1 md:space-x-2"
                  >
