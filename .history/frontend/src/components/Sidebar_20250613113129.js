...
import { DialogTitle, DialogDescription } from './ui/dialog'; // Import for SheetContent's accessibility
...
export default function Sidebar({ isOpen, toggleSidebar, onSelectChat, clearMessages, displayMessageBox }) {
  ...
  const handleNewInteraction = () => {
    ...
    // SheetClose will handle closing if this is inside the Sheet
  };
  ...
  return (
    // The Sheet component needs to live at the top level in App.js or a parent that manages its state
    // Here, we just render the SheetContent when isOpen is true.
    // The SheetTrigger in ChatHeader will control the 'open' prop of the Sheet in App.js.
    // This component is effectively the *content* of the Sheet.
    <Sheet open={isOpen} onOpenChange={toggleSidebar}> {/* Pass isOpen and onOpenChange here */}
      <SheetContent
        side="left" // The sidebar slides in from the left
        // Crucial styling for making it a fixed sidebar on desktop, but overlay on mobile
        className="w-64 bg-zinc-900 border-r-4 border-pink-700 shadow-2xl z-40 flex flex-col py-6 px-4
                   ...
                   md:static md:flex" // md:static md:flex ensures it renders normally on desktop
      >
        {/* Sidebar Header */}
