...
const ScrollingHumorPanel = ({
  ...
  const speed = 80; // Faster scrolling (was 50)
  ...
  return (
    <div
      className={`relative bg-gradient-to-b from-zinc-900 to-zinc-800 h-full overflow-y-auto flex flex-col border-2 ${position === 'left' ? 'border-r-4 border-pink-700/80' : 'border-l-4 border-pink-700/80'} shadow-lg shadow-pink-700/20 z-30`}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      onScroll={handleScroll}
    >
      <div className="flex justify-between items-center p-2 bg-gradient-to-r from-pink-700 to-purple-700 border-b-2 border-pink-500 sticky top-0 z-50">
