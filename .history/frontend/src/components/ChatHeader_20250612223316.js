...
import { ChevronDown, Menu, User, LogIn, LogOut, Flame, Volume2, VolumeX, Medal } from 'lucide-react'; // Added Medal icon

// Shadcn UI Components (assuming they are in src/components/ui)
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from './ui/dropdown-menu';
import { Sheet, SheetContent, SheetTrigger } from './ui/sheet'; // For mobile sidebar overlay
...
export default function ChatHeader({
  ...
  onUnlockAchievement, // New prop
}) {
  const [showLanguageSelection, setShowLanguageSelection] = useState(false);
  const [showProfileOptions, setShowProfileOptions] = useState(false); // Can be replaced by Shadcn DropdownMenu
  const [showVoiceStyleSelection, setShowVoiceStyleSelection] = useState(false); // Can be replaced by Shadcn DropdownMenu
  ...
  const handleDummyLogin = () => {
    ...
    // setShowProfileOptions(false); // No need if using Shadcn Dropdown which closes automatically
  };
    ...
    >
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        <Sheet>
          ...
          {/* SheetContent for Sidebar is handled in App.js */}
        </Sheet>
      ...
      <div className="flex items-center space-x-1 md:space-x-3 absolute left-1/2 -translate-x-1/2">
        <img
          src="[https://placehold.co/40x40/FF00FF/FFFFFF?text=AI](https://placehold.co/40x40/FF00FF/FFFFFF?text=AI)" // Placeholder image URL for the title
          alt="brocodeAI Logo"
          className="h-6 w-6 md:h-8 md:w-8 rounded-full border-2 border-pink-500 animate-pulse-slow"
          onError={(e) => { e.target.onerror = null; e.target.src = "[https://placehold.co/40x40/CCCCCC/000000?text=Logo](https://placehold.co/40x40/CCCCCC/000000?text=Logo)"; }}
        />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-40 md:w-48 bg-zinc-700 rounded-lg shadow-2xl border border-zinc-600 overflow-hidden">
            ...
            {voiceStyles.map((voice) => (
              <DropdownMenuItem
                key={voice.style}
                onClick={() => onSelectVoiceStyle(voice.style)}
