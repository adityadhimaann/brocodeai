...
import { SheetTrigger } from './ui/sheet'; // Import SheetTrigger explicitly
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        {/* SheetTrigger for Sidebar (now correctly wrapped around the button) */}
        <SheetTrigger asChild>
          <motion.button
            // Removed onClick={toggleSidebar} here as SheetTrigger handles the toggle directly
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-40 md:w-48 bg-zinc-700 rounded-lg shadow-2xl border border-zinc-600 overflow-hidden">
            ...
            {voiceStyles.map((voice) => (
              <DropdownMenuItem
                key={voice.style}
                onSelect={() => onSelectVoiceStyle(voice.style)} // Use onSelect for Shadcn DropdownMenuItem
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-40 md:w-48 bg-zinc-700 rounded-lg shadow-2xl border border-zinc-600 overflow-hidden">
            ...
            {languages.map((lang) => (
              <DropdownMenuItem
                key={lang.code}
                onSelect={() => onSelectLanguage(lang.code)} // Use onSelect for Shadcn DropdownMenuItem
                className={`block w-full text-left px-4 py-2 text-xs md:text-sm font-medium transition-colors duration-200
