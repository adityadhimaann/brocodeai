...
import { Laugh, X, AlertTriangle, RefreshCw, Volume2 } from 'lucide-react'; // Added Volume2 icon

export default function ScrollingHumorPanel({ humorItems = [], direction = 'up', position = 'left', isLoading, error, onRefresh, onSpeakText, selectedLanguage, selectedVoiceStyle }) { // New props
  const panelRef = useRef(null);
  const [scrollDuration, setScrollDuration] = useState('100s');
  const [isHovered, setIsHovered] = useState(false);
  const [speakingItemId, setSpeakingItemId] = useState(null); // Track which item is speaking
  ...
  const handleListenClick = async (content, itemId) => {
    setSpeakingItemId(itemId); // Set the ID of the item currently speaking
    await onSpeakText(content, selectedLanguage, selectedVoiceStyle);
    setSpeakingItemId(null); // Clear the ID after speaking
  };
  ...
  return (
    <motion.div
      ...
      className={`hidden md:flex fixed inset-y-0 w-64 md:w-72 lg:w-80 ${position}-0 z-10 flex-col justify-center items-center p-4 overflow-hidden ${panelBg} shadow-2xl ${panelBorder} ${panelRounded}`}
            ...
            {displayItems.map((item, index) => (
              ...
              >
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  disabled={speakingItemId === `${item.type}-${index}`} // Disable if currently speaking
                >
                  <Volume2 className={`h-4 w-4 ${speakingItemId === `${item.type}-${index}` ? 'animate-pulse' : ''}`} />
                  <span>{speakingItemId === `${item.type}-${index}` ? 'Speaking...' : 'Listen'}</span>
                </motion.button>
              </div>
            ))}
          </div>
