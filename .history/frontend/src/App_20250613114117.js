...
// Shadcn UI components for Sheet (Dialog/SheetTitle/SheetDescription are part of dialog component)
...
// REMEMBER TO REPLACE THESE WITH YOUR ACTUAL FIREBASE PROJECT WEB APP CONFIG
...
export default function App() {
  ...
  const unlockSarcasticAchievement = useCallback(async () => {
    ...
    try {
        const response = await fetch('http://localhost:5002/unlock_achievement', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-DEBUG-USER-ID': userId }, // Added debug header
            body: JSON.stringify({ user_id: userId, language: selectedLanguage })
        });
        if (!response.ok) {
            const errorData = await response.json();
  ...
  // Removed unused panelWidth, sidebarWidth, mainContentMx variables.
  // The CSS variables are already defined in index.html for responsiveness.
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      {/* The Sheet and SheetContent define the context for SheetTrigger/SheetClose */}
      <Sheet open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
        {/* SheetTrigger is placed in ChatHeader to open this Sheet */}
        {/* If you want a button to open the sidebar, use SheetTrigger here */}
        {/* Example: 
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out
        ...
        <ChatHeader
          ...
          toggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} // This now directly controls Sheet's open prop
