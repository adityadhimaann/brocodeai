  ...
  return (
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <Sheet open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
        ...
        {/* No absolute position here, it's within the header's flex flow */}
        {/* This trigger will be shown only on mobile and will be styled in ChatHeader */}
        {/* Actual SheetTrigger is now removed from App.js as it's not needed directly here */}
        ...
        <SheetContent
          ...
          className="w-64 bg-zinc-900 border-r-4 border-pink-700 shadow-2xl z-40 flex flex-col py-6 px-4
                     ...
                     md:hidden" /* Only visible on mobile, hidden on md and up */
        >
      ...
      {/* Main Content Area - this is the central column wrapper */}
      <div className="flex flex-col flex-1 h-full w-full relative z-20"> {/* Flex column takes full height */}
          ...
          <ChatHeader
            ...
            toggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} // Pass toggle function to ChatHeader for its Menu button
          ...
          {/* Main Chat Content below the header, adjusted for side panels on desktop */}
          <div className="flex-1 flex justify-center p-2 sm:p-4 overflow-hidden"
               style={{
                 // Adjust padding dynamically based on sidebar open state and panel widths
                 paddingLeft: `max(0.5rem, var(--panel-width-val, 0px))`, /* Default 0.5rem, then panel width for desktop */
                 paddingRight: `max(0.5rem, var(--panel-width-val, 0px))`,
                 // If sidebar is open on mobile, push content right.
                 // On desktop, the fixed sidebar pushes the content.
                 marginLeft: isSidebarOpen ? 'var(--sidebar-width)' : '0px', // Only for desktop if sidebar is open (visual push)
               }}
          >
          ...
          <div className="flex justify-center p-2 pt-0 sm:p-4 sm:pt-0"
               style={{
                 paddingLeft: `max(0.5rem, var(--panel-width-val, 0px))`,
                 paddingRight: `max(0.5rem, var(--panel-width-val, 0px))`,
                 marginLeft: isSidebarOpen ? 'var(--sidebar-width)' : '0px', // Similar logic for input
               }}
          >
            <MessageInput
                input={input}
