...
export default function App() {
  ...
  const languages = [
    ...
    { name: 'Hindi', code: 'hi' },
    { name: 'Hinglish', code: 'hinglish' }, // New Hinglish option
  ...
  useEffect(() => {
    if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
      ...
      // For Hinglish, use 'hi-IN' as the language code for speech recognition as it's the closest available
  ...
  // This assumes the panels have fixed widths. If panels are fluid, this would need adjustment.
  const mainContentMx = `md:ml-[${panelWidth}] md:mr-[${panelWidth}]`; // Left and right margins
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      {/* Main Content Area */}
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out ${mainContentMl} ${isSidebarOpen ? '' : 'md:ml-80'} md:mr-80 z-20`}> {/* Adjusted margins */}
        <ChatHeader
          languages={languages}
