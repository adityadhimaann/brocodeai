...
export default function App() {
  ...
  // Set default language to 'hinglish'
  ...
  const [sarcasmSuggestions, setSarcasmSuggestions] = useState([
    // These suggestions are static, could be dynamic from backend
    "Oh, how original.",
  ...
  // Define supported Indian languages with their codes, with Hinglish prominently listed
  const languages = [
    { name: 'Hinglish', code: 'hinglish' }, // Now default and first in list
  ...
  useEffect(() => {
    if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
      ...
      // For Hinglish, use 'hi-IN' for speech recognition as it's the closest available
  ...
  const handleSelectChat = (chatId) => {
    ...
    setMessages([]); // Clear current messages when a dummy chat is selected
  ...
  const panelWidth = 'w-64 md:w-72 lg:w-80'; // Consistent width for panels
  const sidebarWidth = 'w-64'; // Sidebar width
  const mainContentMl = isSidebarOpen ? 'md:ml-64' : 'ml-0'; // Sidebar pushes content
  // Calculate horizontal margin for main content to fit between panels
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <Sidebar
        ...
        clearMessages={clearMessages} // Pass clearMessages function
        displayMessageBox={displayMessageBox} // Pass displayMessageBox function
      />
