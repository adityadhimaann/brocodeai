...
export default function App() {
  ...
  // NEW: State for selected voice style
  const [selectedVoiceStyle, setSelectedVoiceStyle] = useState('default'); // 'default', 'sarcastic', 'hot_male', 'hot_female'
    ...
    try {
      const response = await fetch('http://localhost:5002/chat', {
        ...
        body: JSON.stringify({
          ...
          language: selectedLanguage, // Use selectedLanguage from state
          voice_style: selectedVoiceStyle, // Pass selected voice style
  ...
  // NEW: Function to speak arbitrary text via backend
  const speakArbitraryText = useCallback(async (text, lang, voiceStyle) => {
    ...
    try {
      ...
      if (result.audio) {
        ...
        audio.play().catch(e => console.error("Error playing arbitrary audio:", e));
      }
    } catch (error) {
      console.error('Error speaking arbitrary text:', error);
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-inter antialiased overflow-hidden relative">
      ...
      <ScrollingHumorPanel
        ...
        onSpeakText={speakArbitraryText} // Pass speak function
        selectedLanguage={selectedLanguage}
        selectedVoiceStyle={selectedVoiceStyle} // Pass selected voice style
      />
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ease-in-out
        ...
        <ChatHeader
          ...
          selectedVoiceStyle={selectedVoiceStyle} // Pass to header
          onSelectVoiceStyle={setSelectedVoiceStyle} // Pass setter to header
          toggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)}
