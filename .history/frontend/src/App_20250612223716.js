...
// NEW: Import Loader2 for loading spinner
import { Loader2 } from 'lucide-react';

// NEW: Firebase imports for client-side
...
export default function App() {
                ...
                if (__initial_auth_token) {
                    try {
                        ...
                        try {
                            ...
                            displayMessageBox("Firebase authentication failed. Chat history will not persist.", "error");
  ...
  const handleLogin = useCallback((username, password) => {
    ...
    if (auth) {
        signInWithCustomToken(auth, __initial_auth_token || 'dummy_token_for_dev') // Using dummy token for dev if not provided
            .then((userCredential) => {
