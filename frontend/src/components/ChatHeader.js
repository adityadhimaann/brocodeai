...
export default function ChatHeader({
  ...
  onUnlockAchievement,
  // FIX: Using new persona mode props correctly
  ...
  // This state is now redundant as DropdownMenu handles its open state internally
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-48 md:w-56 bg-zinc-700 rounded-lg shadow-2xl border border-zinc-600 overflow-hidden">
            ...
            {personalityModes.map((mode) => (
              <DropdownMenuItem
                key={mode.mode}
                onSelect={() => onSelectPersonalityMode(mode.mode)} /* FIX: Comment moved outside attribute */
